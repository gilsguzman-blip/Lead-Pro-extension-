// (v9.7.379-dev / v9.7.377) FINAL-AUDIT changes in this file: F4 hidden frames no longer write
// the lp_lastActiveLead beacon (cross-tab hijack), F2 beacon carries scraperVersion for popup-side
// enforcement, F8 date-selector canary. scraperVersion stays v9.7.51 — beacon field addition and
// canary are non-payload changes (v9.7.307 / v9.7.374 precedent).
// ─────────────────────────────────────────────────────────────────
...
(function() {
  ...
  // (v9.7.379/377 F8) SELECTOR CANARY. Every date read intentionally uses VinSolutions' own DOM
  // typo '.notes-and-hsitory-item-date'. If Cox ever fixes their typo, every date silently reads
  // empty and all date-gated logic (transcript cutoff, opt-out freshness) opens with no visible
  // failure. Detect that day the moment it happens.
  try {
    if (noteEls.length > 0 && document.querySelectorAll('.notes-and-hsitory-item-date').length === 0) {
      console.warn('[LP SELECTOR CANARY] (content.js) notes present but ZERO .notes-and-hsitory-item-date matches — Cox may have fixed their DOM typo; date-gated logic is compromised. Update selectors.');
    }
  } catch(e) {}
  const histLines = [];
  ...
  // (v9.7.462/457 fix) Prefer the "City, ST 12345" shape — the old first-5-digit-run match
  // could grab a mileage ("52000") or an order number as the ZIP and misclassify distance.
  // Shaped match first (the same shape the popup inlineScraper has used since v9.7.226);
  // the loose match is retained only as a fallback so a bare on-page ZIP is still caught.
  // NOTE: content.js scrape results are not consumed by the current popup flow (see the
  // v9.7.46 note below) — fixed for consistency with the live inlineScraper. Non-payload;
  // scraperVersion unchanged per the v9.7.307/374 precedent.
  var customerStateZipMatch = TEXT.match(/\b([A-Z]{2})\s*,?\s*(\d{5})(?:-\d{4})?\b/);
  var customerZip = customerStateZipMatch ? customerStateZipMatch[2]
    : ((TEXT.match(/\b(\d{5})(?:-\d{4})?\b/) || [])[1] || '');
  var customerState = customerStateZipMatch ? customerStateZipMatch[1] : '';
  ...
  // (v9.7.46 architecture) NO full storage write here — frame results stay in
  // memory only to avoid cross-frame race conditions.
  //
  // (v9.7.145) EXCEPTION: write a narrow "last active lead" beacon when this
  // frame is a rims2 lead frame with a confirmed autoLeadId. This is a single
  // small key, not the full scrape payload, so it doesn't cause the race.
  // Popup reads this at GRAB time as the most reliable signal for which lead
  // the agent last navigated to — eliminating the ECCS/frame-vote ambiguity
  // that affects same-customer multi-lead scenarios.
  // (v9.7.307) Do NOT beacon service-record frames. A customer with service history has service
  // entries that render as rims2 lead frames with their own AutoLeadID, but their source is a dead
  // DMS / service scheduler (Autosoft = decommissioned DMS, xTime/xTimeLegacy = service scheduling),
  // never a live sales source. Without this guard such a frame writes itself in as "last active
  // lead," the popup trusts it, the real sales lead is rejected as a sibling mismatch, and the VOI
  // bleeds back to the owned service vehicle. leadSource is the reliable tell — gate the beacon on it.
  var _isServiceSrc = /\b(?:autosoft|xtime)/i.test(leadSource || '');
  // (v9.7.379/377 F4) HIDDEN FRAMES DO NOT BEACON. chrome.storage.local is global across tabs
  // and windows; the beacon carried no visibility or tab identity, so a hidden other-rooftop
  // frame loading on the same customer record (or a lead loading in a second VinSolutions
  // window) could write itself in as "last active lead" within the 10s trust window and hijack
  // the next grab — and because the beacon short-circuits the entire pre-scrape, the Strategy
  // 1.5 visible-frame preference never got the chance to correct it. A display:none iframe
  // reports zero inner dimensions from inside (v9.7.291 precedent) — only a VISIBLE frame may
  // beacon.
  // (v9.7.379/377 F2) The beacon now also carries scraperVersion; the popup rejects a version
  // mismatch, closing the stale-content-script window after Chrome auto-updates the extension
  // (MV3 does NOT reload content scripts in already-open tabs).
  var _isVisibleForBeacon = !!(window.innerWidth > 0 && window.innerHeight > 0);
  if (isLeadFrame && autoLeadId && URL.includes('rims2.aspx') && !_isServiceSrc && _isVisibleForBeacon) {
    try {
      chrome.storage.local.set({
        'lp_lastActiveLead': {
          leadId: autoLeadId,
          dealerId: dealerId || dealerIdFromUrl || '',
  ...
  function patchIdentityIntoResult() {
    ...
    // (v9.7.354) THE source-unknown fix for pure-SPA count=0 leads. The initial scrape reads
    // leadSource ONCE at document_idle; if the LeadSourceName span renders late (the same lag that
    // delays identity fields — which is why this retry ladder exists), the empty source was never
    // refreshed because this ladder only re-extracted identity, not source. Re-read it here so a
    // late-rendered source recovers the same way agent/name do. PROBE: log what the span looks like
    // so a live count=0 grab confirms whether source is readable at late render (vs genuinely absent).
    if (!result.leadSource) {
      _rowCache = null; // re-query fresh DOM (mirrors reExtractIdentity)
      var _freshSrc = firstId([
        'ActiveLeadPanelWONotesAndHistory1__LeadSourceName',
        'ActiveLeadPanel1__LeadSourceName'
      ]) || firstSel([
        'span[id*="LeadSourceName"]',
      ...
      var _srcSpan = document.querySelector('span[id*="LeadSourceName"], [id*="LeadSourceName"]');
      console.log('[LP SOURCE-PROBE] result.leadSource empty | span present:', !!_srcSpan,
        '| span id:', (_srcSpan && _srcSpan.id) || '(none)',
        '| re-extract value:', JSON.stringify(_freshSrc || ''));
      if (_freshSrc) {
        result.leadSource = _freshSrc; changed = true;
        console.log('[LP SOURCE-RECOVERY] late-render source recovered:', JSON.stringify(_freshSrc));
      }
    }
    return changed;
  }
  ...
  chrome.runtime.onMessage.addListener(function(msg, sender, sendResponse) {
    ...
    // (v9.7.462/457 fix) Only signal an async response for the message type we actually
    // respond to — the unconditional `return true` promised a response on every message
    // and produced "message port closed" console noise.
    return !!(msg && msg.type === 'LEADPRO_SCRAPE_NOW');
  });
