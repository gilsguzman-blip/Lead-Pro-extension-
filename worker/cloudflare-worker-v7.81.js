/**
 * v7.81: THE SMS REFINE PASS GOES TO GPT-6 LUNA AT REASONING EFFORT 'none' TOO (Gil, 9/24: "yes ship
 *   it as v7.81"). Worker only; extension, reporter and dashboard unchanged. Either deploy order.
 *
 *   THE TEST (9/24, through v7.80, edge cache bypassed). The 30 first-pass SMS drafts from the 11-lead
 *   none run, each refined by the SHIPPED v9.7.715 buildSystemPromptSmsRefine + _lpBuildSmsRefinePrompt,
 *   3 times at each effort with the order alternated: 180 calls, all 200, all with an SMS.
 *                                            low        none
 *     round-trip median                      3,817 ms   1,725 ms
 *     production guards would revert         0 / 90     0 / 90
 *     opens on the customer's first name     90 / 90    90 / 90
 *     source kept (15 first drafts named it) 15 / 15    15 / 15
 *     invented time / money / day / date     0          0
 *     appointment times dropped (9 had them) 9          5     (all one lead; both arms defer to Friday)
 *     non-ASCII introduced                   0          0
 *     semicolons                             0          4     (the one cost seen; style, not fact)
 *     fell to the 5.6 tier                   0          1
 *   "Missing question" (low 2, none 7) was inspected: every one is on two Toyota leads whose FIRST
 *   drafts deliberately ask nothing; low added a question to 16 of those 18, none kept the no-ask
 *   intent more often. Not a defect.
 *
 *   THE CHANGE. v7.79's draftEffort applies when the caller sent no effort, the contract is 'draft',
 *   and the system prompt carries the cache sentinel. The refine pass has no sentinel, so it also
 *   qualifies now when its system prompt carries the refine marker below -- a fixed sentence the
 *   extension's buildSystemPromptSmsRefine emits (v9.7.672 onward):
 *       "An email to this customer has ALREADY been written and is going out."
 *   Only tiers with draftEffort take it (gpt-6-luna); the 5.6 fallback and the nano emergency tier
 *   stay on REASONING_EFFORT. The translation call carries neither marker and stays on 'low' -- it
 *   was not in either test. Explicit efforts are always honoured.
 *   perf: rows keep fullDraft as it was (the refine is NOT a full draft) and gain smsRefine, so
 *   /perf's otherDraftCallsByEffort can be read for this pass.
 *   TO REVERT: drop the `|| _isSmsRefine` term.
 *   VERIFIED: tests/worker-v781.test.js, 15 assertions on the shipped handler and both extension builds
 *   (6 [new], 7 [control], 2 [link] -- the extension's refine prompt really carries the marker, the
 *   main prompt does not); against v7.80 the 6 [new] fail and the other 9 pass. worker-v777's perf
 *   field list gains smsRefine. run-all: 141 suites, 6,472 assertions, 0 failed; pointed at v7.80 the
 *   only failures are worker-v781's own 6 [new].
 */
/**
 * v7.80: /perf's draftsByEffort COUNTS FULL DRAFTS ONLY. Worker only; either deploy order.
 *   v7.79's tally took every primary-tier 'draft'-contract call. The SMS refine pass and translation
 *   are 'draft' contract too, and they stay on 'low', so on the first real traffic (9/23, 02:32 and
 *   02:49 UTC) the "low" bucket was two refine passes (3.2-3.7s on a ~7.4k-char prompt) set against two
 *   full drafts under "none" (3.7-4.3s on ~33k). Read as none-vs-low it said the opposite of the test.
 *   1  perf: rows gain fullDraft: the draft contract AND the cache sentinel, the same test that decides
 *      v7.79's draftEffort (only buildSystemPrompt emits the sentinel: the main draft and the voicemail
 *      button).
 *   2  draftsByEffort holds full drafts only; the other draft-contract calls go to a new
 *      otherDraftCallsByEffort beside it. Rows v7.79 wrote have no fullDraft flag and are sorted by
 *      sysChars >= 15,000 (full ~33k, refine ~7.4k).
 *   LEFT ALONE: every generation path, the efforts sent, the cascade, the subject lift, and every
 *   other perf field and tally.
 *   VERIFIED: tests/worker-v780.test.js, 8 assertions on the shipped handler; against v7.79 the 5
 *   [new] fail and the 3 [control] pass. worker-v779's /perf check now pins only the "none" bucket
 *   and the lift count (the "low" bucket is what this build corrects), and worker-v777's perf field
 *   list gains fullDraft. run-all: 134 suites, 6,249 assertions, 0 failed. Pointed at v7.79, the only
 *   failures are worker-v780's own 5 [new] assertions.
 */
/**
 * v7.79: FULL DRAFTS GO TO GPT-6 LUNA AT REASONING EFFORT 'none' (Gil, 9/24: "if none holds up, ship it
 *   as v7.79"). Worker only; extension, reporter and dashboard unchanged. Either deploy order.
 *
 *   THE TEST (9/23, 8:03-8:10 PM Central, through v7.78, edge cache bypassed). The 11 real leads of
 *   the v9.7.701 M4 harness (two per rooftop plus the Audi Lafayette dump), their prompts in the
 *   M4 layout, sent 3 times at each effort with the order alternated: 66 calls, all 200, all drafts.
 *                               low       none
 *     worker latency median     5,433 ms  3,024 ms   (-44%)
 *     worker latency p90        6,686 ms  3,692 ms
 *     worker latency max        7,553 ms  4,101 ms
 *     output tokens median      537       275        (reasoning: ~300 -> ~23)
 *   'none' was faster on 10 of 11 leads.
 *   CONTENT, read side by side: equally specific and on-rule. Zero lists, zero volunteered payment
 *   or out-the-door figures, zero non-English on both sides. Where the two differed on a fact,
 *   'none' was right both times: it quoted the lead's exact trade mileage where 'low' rounded it
 *   wrongly, and it answered the customer's AWD question the prompt tells it to address. Trade
 *   mentions rose 2 -> 5, all on leads whose own record carries a TRADE-IN.
 *   STRUCTURE is where 'none' is sloppier, and every case is caught:
 *     - 1 of 33 abandoned the email after 1.5s (a 1-char field, no voicemail). The degenerate
 *       guard rejected it and 5.6 Luna answered at 'low' (degen row 01:06:22 UTC).
 *     - 1 of 33 double-encoded the email. v7.73's repair unwrapped it (degen row 01:04:50 UTC).
 *     - 7 of 33 had no top-level subject (2 of 33 at 'low'). The extension takes the subject ONLY
 *       from that key and strips a leading Subject: line from the body, so the model's subject was
 *       thrown away and a generic one built. Item 3 lifts it: 6 of the 7 recoverable (both at 'low').
 *   Not tested at 'none', so NOT changed: the SMS refine pass, translation, the recovery tiers.
 *
 *   WHAT CHANGED
 *   1  gpt-6-luna's cascade row gains draftEffort: 'none'. It applies ONLY when the caller sent no
 *      reasoningEffort, the contract is 'draft', and the system prompt carries the cache sentinel
 *      (only buildSystemPrompt emits it: the main draft and the voicemail button). An explicit
 *      effort, 'low' included, is honoured as before. REASONING_EFFORT stays 'low', so escalation,
 *      the recovery-tier clamp and the START line are unchanged; an [EFFORT] line says when the
 *      tier default applied. 5.6 Luna and 5.4-nano keep 'low': a draft that falls back gets the
 *      effort those models were measured on.
 *   2  The phone-ask regen edits at the effort of the draft it edits (was the request's baseline).
 *   3  liftEmbeddedSubject: a draft with no usable top-level subject whose email starts with a
 *      "subject:" line (any case), or is an object with its own subject, gets that subject as the
 *      top-level key. Nothing else changes; the email is returned exactly as written. Logged as
 *      SUBJECT-LIFTED with the source and length only. It also recovers the 'low' cases.
 *   4  perf: rows gain `effort` (what was sent) and `subjectLifted`. /perf tallies gain
 *      draftsByEffort {calls, latencyMedian} for primary-tier drafts, and a subjectLifted count.
 *      degen: rows record the effort actually sent (was the request's baseline).
 *
 *   TO REVERT: delete draftEffort from the gpt-6-luna row. Items 3 and 4 are independent of it.
 *   WATCH: /degenerate byModel for gpt-6-luna rejected rows (the abandonment rate was 1 in 33 on 33
 *   calls, which is a wide estimate), /perf draftsByEffort, and byTier fallback counts.
 *
 *   LEFT ALONE: the cascade, timeouts, budget, REASONING_EFFORT, escalation, the cache fields, the
 *   degenerate guard and v7.73 repair, the classifier, everything v7.77 and v7.78 added.
 *
 *   VERIFIED: tests/worker-v779.test.js, 21 assertions on the shipped handler. Against v7.78 all 10
 *   [new] fail and all 11 [control] pass. Three existing assertions updated with notes:
 *   worker-v778 (effort left out of its primary payload shape), worker-v777 (the perf row's field
 *   list gains the two fields), regen-effort (its scope window widened past the new comment).
 *   run-all: 131 suites, 6,193 assertions, 0 failed. Pointed at v7.78, the only failures are
 *   worker-v779's own 10 [new] assertions.
 */
/**
 * v7.78: THE CASCADE MOVES DOWN ONE RUNG FOR GPT-6 LUNA (Gil, 9/23). Worker only; extension v9.7.706,
 *   reporter v1.22 and the dashboard are unchanged (none of them names a model). Deploys in either
 *   order against any extension build: the request and response shapes are untouched.
 *
 *   WHAT CHANGED
 *   1  MODEL_CASCADE  primary gpt-6-luna, fallback gpt-5.6-luna, emergency gpt-5.4-nano-2026-03-17
 *      (was gpt-5.6-luna / gpt-5.4-nano / gpt-4.1-nano). Each model moved with its own settings:
 *        gpt-6-luna    3500 tokens, 12000ms, reasoning, cacheBreakpoints ON. Its model page: effort
 *                      none..max (the same six as 5.6), prompt caching supported, cache writes 1.25x,
 *                      and GPT-6 takes prompt_cache_options.ttl '30m' in place of retention.
 *        gpt-5.6-luna  3500 tokens, reasoning, cacheBreakpoints ON (as it has run since v7.44).
 *                      Takes the FALLBACK SLOT's 8000ms, not its own 12000ms (see BUDGET).
 *        gpt-5.4-nano  3500 tokens (its own figure; 4.1-nano's 2000 would starve a reasoning model),
 *                      no temperature, reasoning, cacheBreakpoints OFF so it gets
 *                      prompt_cache_retention '24h', a plain system string, and the sentinel stripped.
 *                      Timeout cap 8000 (its own figure; was 5000 for 4.1-nano). See BUDGET.
 *   2  TIER_EFFORTS gains gpt-6-luna. gpt-4.1-nano's entry stays, so putting it back is one line.
 *   3  A MODEL-SPECIFIC 401/403/404 NO LONGER ENDS THE CASCADE. `fatal` stopped the ladder on any
 *      401/403/404. Right for a bad key, which every tier shares; wrong for "this project cannot use
 *      this model" (404 model_not_found, or a 403 naming the model), where the next tier is a
 *      different model that works. Until now every tier was a model the project already ran, so it
 *      never mattered. With a brand-new primary it is the likeliest deploy-day failure: if gpt-6-luna
 *      is not yet enabled on the project, v7.77's rule would have served SAFE_FALLBACK on EVERY
 *      request and never tried 5.6 Luna. Now: error.code 'model_not_found' or a message naming a
 *      model -> not fatal, the next tier runs, and the FAIL line still carries the status.
 *
 *   BUDGET (unchanged total, 24000ms). Worst case, primary and fallback both time out:
 *   12000 + 8000 + 3700 = 23700ms, all three tiers attempted, exactly as since v7.39. The fallback
 *   slot cannot exceed ~9200ms or the emergency tier stops being admitted after a primary timeout,
 *   so 5.6 Luna runs at 8000ms there. ITS P99 AS PRIMARY WAS ~9.6s (P95 ~8.2s, 7/31), so roughly one
 *   in twenty FALLBACK calls that run long will time out into the emergency tier. Fallback calls
 *   are rare, and /perf shows every one. The emergency cap of 8000 only matters when the tiers
 *   above failed FAST (the 7/31 17:06 shape, all in <250ms); after timeouts the budget clamps it to
 *   3700 either way. An escalated regen keeps the v7.64 trade: gpt-6-luna at 'high' for 18000ms,
 *   5.6 Luna at 'low' for 5700ms, no emergency attempt.
 *
 *   WHAT THIS GIVES UP, stated plainly: every tier is now a reasoning model. v7.39 put gpt-4.1-nano
 *   (non-reasoning) back at the bottom on purpose, after the 7/31 17:06 event failed all three
 *   GPT-5 tiers in 73-243ms, and because the v7.30/W2 reasoning-burn failure shape cannot empty a
 ...
 *   regen guard, degenerate handling, and everything v7.77 added.
 *
 *   VERIFIED: tests/worker-v778.test.js, 20 assertions. It drives the shipped handler with the model
 *   API scripted by call index and a virtual clock, so the real timeout arithmetic runs. Against
 *   v7.77, all 11 labelled [new] fail and all 9 labelled [control] pass. Four existing assertions
 *   pinned the old cascade and were updated, each with a note: safe-fallback-contract (tokens 3500
 *   on all three; the family-diversity check, which would have passed on name-splitting alone, now
 *   records that every tier reasons; the 400-not-fatal pin accepts the new fatal line),
 *   regen-effort (cacheBreakpoints on both Luna tiers, read from the cascade rows), worker-smoke
 *   (the v7.61 check targets the pre-5.6 tier, now third), worker-v777 (its byte-identity control
 *   normalises the model name). run-all: 130 suites, 6,172 assertions, 0 failed.
 */
/**
 * v7.77: SEVEN ITEMS FROM THE v7.76 REVIEW. Pairs with extension v9.7.706 (items 4 and 7); either can
 *   deploy first. Reporter unchanged at v1.22 -- see item 3. Privacy posture unchanged (v9.7.489): every
 *   new field is an id, a length, a count, a timing or a build string; no prompt text, customer data or
 *   draft content is logged or stored.
 *
 *   1  /list-licenses LISTS LICENSES ONLY. It did one unprefixed list() -- the first 1,000 keys of a
 *      namespace shared with commit:, degen:, feedback:, invite: and now perf: rows -- filtered out only
 *      feedback:, then did one KV read per remaining key and returned each parsed row as a "license".
 *      THE REVIEW'S CLAIM WAS WRONG IN ONE RESPECT: real licenses could NOT fall outside the first 1,000
 *      keys. KV lists in UTF-8 byte order; 'L' (0x4C) sorts before every lowercase byte, so every LP- key
 *      comes before every telemetry key, and a license is crowded out only if there are more than 1,000
 ...
 *      backfill wrote under the key as typed). Both are INCLUDED, flagged keyFormat, and named in a new
 *      keyFormats field, never silently dropped. [LIST-LICENSES] logs the counts. Deploy and call it once:
 *      keyFormats.other and keyFormats.lowercase answer "are there older formats" on the real data.
 *   2  /feedback/summary byScenario passes e.signal to _tallyBucket (v7.51 gave every other bucket it;
 *      scenario explicitDown was always 0) and applies the v7.52 abandoned->incomplete rewrite before the
 *      loop, so scenario buckets reconcile with the top-line totals beside them.
 *   3  CENTRAL DAYS for /feedback/summary (date= and its default) and /feedback/range (day list, per-day
 *      buckets, totals), matching /feedback/drafts and /degenerate. Rows are placed by their own ts (the
 *      field /feedback/drafts filters on), falling back to the key's server timestamp; each UTC-date prefix
 *      is read once per request. Both responses gain tz:"America/Chicago". A malformed date is now a 400.
 *      THE REPORTER NEEDS NO CHANGE, and that corrects the premise: reporter v1.22 has ZERO reads of
 *      /feedback/summary or /feedback/range. It reads feedback: rows from KV directly and has filtered
 *      them to the Central day since v1.7; the "stay on their existing UTC basis" note in v7.29 describes
 *      these two endpoints, not the email. So the email was already Central, and this build makes the
 *      endpoints AGREE with it. The endpoint consumer is dashboard/index.html, whose ranges all end on
 *      Central today and which already asks for to+1 to catch the UTC-next-day evening; against v7.77 the
 *      extra day is tomorrow and empty, against v7.76 it does what it always did, so it is correct with
 *      either proxy and is not changed. TRANSITION: nothing is written differently, so there is no
 *      transition DAY -- every day re-renders on the Central basis the moment this deploys. What moves is
 *      the 7 PM-midnight Central slice (CDT; 6 PM-midnight CST), from the next day to its own day: 3.2% of
 *      the 496 feedback rows in the 9/1-9/22 exports, 0-19% on any one day (19.2% on 9/17). A single-day
 *      summary moves by that day's evening minus the previous day's; a range loses the evening before its
 *      first day. Numbers remembered from before the deploy can differ by those amounts; the reporter's
 *      emails will not, because they were already on this basis.
 *   4  THE BUILD THAT ASKED. body.extensionVersion (v9.7.706+) is clamped to 24 characters of
 *      [A-Za-z0-9._+-], appended to the START line as ` ext=<ver>`, and stored on degen: rows and perf:
 *      rows. Absent -- any older extension -- the START line and degen: rows are byte-identical to v7.76.
 *   5  A CLASSIFIER FAILURE IS NOT A "NO". classifyPhoneAsk returned false on a non-OK response or a
 *      throw, so a broken classifier printed sms=NO -- v7.64 hid a 400 on every call that way. It now
 *      returns { state: 'yes'|'no'|'failed' } with the HTTP status or error; a failure prints its own
 *      `CLASSIFY FAILED sms=HTTP 400 ...` line, the diag says FAILED, classifyFailed=true (so the existing
 *      edge-cache gate skips it), and the PREFILTER shadow line says FAILED rather than comparing against
 *      a verdict that does not exist. Behaviour is unchanged: a failed field counts as no flag and the
 *      primary draft is kept.
 *   6  DURABLE PERFORMANCE COUNTERS. One row per generation, perf:<ts>:<requestId>, 14-day TTL, written
 *      on ctx.waitUntil inside its own try like degen: rows; one key per row, never a shared counter (KV
 *      allows one write per key per second). Written at every exit: success, an exhausted cascade
 *      (tier 'exhausted') and an edge-cache HIT (tier 'edge'), so the HIT count can be non-zero. Fields:
 *      ts, requestId, extensionVersion, contract, tier, model, latency, total, sysChars, userChars,
 *      promptTokens, cachedTokens, writtenTokens, edgeCache (HIT/MISS/BYPASS), classifier
 *      (ran/skipped/failed), prefilterWouldSkip, prefilterMiss, regenerated, regenRejected (boolean; the
 *      reason stays in the log). ~430 bytes. NEW director-gated GET /perf?date=YYYY-MM-DD (Central day)
 *      returns the rows and tallies: calls per tier and per contract, edge-cache HIT/MISS/BYPASS,
 *      classifier ran/skipped/failed, prefilter wouldSkip rate and miss count, and cached/uncached tokens
 *      (median, mean, hit count) for primary-tier DRAFTS split at sysChars 30,000 -- pre-M4 ~20-25k vs
 *      post-M4 ~32.5k. perf: is lowercase, so item 1 never lists it. VOLUME: the 9/11 and 9/17 exports
 *      show ~86-92 generations/hour in the afternoon, ~60% drafts and ~40% fact probes; over a ~12-hour
 *      day that is roughly 700-1,100 perf writes on a weekday (~30,000/month), ~6 MB held at the 14-day
 *      TTL. On the Workers Paid plan that is ~3% of the 1M included monthly writes. On the FREE plan the
 *      KV limit is 1,000 writes per DAY for the whole account, which this alone could exceed and which
 *      the existing feedback:/commit: writes already share -- so env.PERF_ROWS='off' stops the writes and
 *      'draft' keeps only the draft contract (~60%). Default 'all'. Check the plan before deploying.
 *   7  SAFE_FALLBACK_TEXT IS NEUTRAL. It asked "would today or tomorrow work better to come in?" on
 *      every lead and carried unfilled [Agent] / [Store] placeholders. Now: "Thanks for reaching out. I'm
 *      pulling your information together and will follow up shortly." in all three fields, no brackets,
 *      no visit. Still three fields, so an older client parses it; v9.7.379+ never renders it and
 *      v9.7.706 shows the agent a failure notice instead.
 *
 *   NOT IN THIS BUILD, as agreed: prefilter gating and edge-cache removal wait for a week of /perf data.
 *   No change to the cascade, timeouts, cache breakpoints, regen guard or degenerate handling.
 *
 *   VERIFIED: tests/worker-v777.test.js, 52 assertions, drives the shipped handlers against a stubbed
 *   KV that lists in byte order and pages with a cursor. Against v7.76, 39 fail; the 13 that pass are
 *   12 labelled controls (unchanged behaviour, byte-identity when absent) and 1 labelled finding (the
 *   KV byte order). safe-fallback-contract.test.js inverts one assertion on purpose (it pinned the
 *   placeholders as present). commit-persistence's TDZ scan stays clean: the perf state is built in
 *   helpers outside that span. run-all: 129 suites, 6,152 assertions, 0 failed.
 */
/**
 * v7.76: A FEEDBACK ROW COULD NOT SAY WHICH BUILD WROTE IT, AND THAT COST TWO WRONG DIAGNOSES.
 *   The row carries the rating, the signal, the chips and the drafts, and nothing at all about the
 *   extension that produced it. Twice in three days that turned a one-field filter into a
 *   deduction, and the second deduction was wrong:
 *     9/18 - three rows carried drafts.final and drafts.prior with no drafts.onScreen. Since the
 *            shipped capture writes all three as ONE object literal, the only available conclusion
 *            was "a build older than v9.7.674 is in the field". That happened to be correct.
 *     9/19 - five rows leaked a customer first name. I attributed it to that same stale build.
 *            It was not: 37 of 37 rows that day carried onScreen, so every one was current, and
 *            the real cause was the prior draft being scrubbed after clearFields() had nulled
 *            lastScrapedData (fixed extension-side in v9.7.683). Chasing the stale build would
 *            never have stopped it.
 *   One field ends that guessing. Stored the same way workerRequestId has been since v7.58 --
 *   clamped, and OMITTED ENTIRELY when absent, so rows from a pre-v9.7.684 extension keep exactly
 *   the shape they have today and `extensionVersion` present is itself the marker that a row can
 *   be attributed. It is a build string, not customer data, so it sits inside the v9.7.489 posture
 *   without argument -- the same reasoning v7.56 used to store note TYPE.
 *   REQUIRES NO EXTENSION CHANGE TO BE SAFE: an older extension simply sends nothing and the field
 *   is absent. Pairs with extension v9.7.684, which starts sending it.
 *
 * v7.75: A REPAIR THAT MADE IT WORSE, ACCEPTED BECAUSE THE ONLY TEST WAS THAT THE CALL SUCCEEDED.
 *   LIVE, 9/11, request 7e0d2e6b — Roy Ballard, Audi Lafayette, agent Jordyn Guzman. Gil: "Very
 *   short response for this agent." What reached her was an SMS consisting of the customer's first
 *   name and the signature, and a one-sentence email. The extension log alone could not say whether
 *   the model sent that or something downstream dropped it; the worker log settles it.
 *
 *     PRIMARY OK  gpt-5.6-luna 4641ms finish=STOP
 *     CLASSIFY    sms=NO email=YES phoneOnFile=true
 *     PREFILTER   shadow pf(sms=true email=true) model(sms=false email=true)
 *     REGEN       triggered (email flagged)
 *     REGEN OK    gpt-5.6-luna 4526ms
 *     FINAL       regenerated=true flagged=true classifyFailed=false
 *
 *   THE MODEL DID NOT SEND THAT DRAFT. The regen did. The email asked for a phone number already on
 *   file, which is precisely what the regen exists to strip, and its revision prompt says "remove
 *   ONLY the phone-number request and keep every other field, sentence, and status fact identical".
 *   The model returned a gutted object instead.
 *
 *   THE PROOF IS THE CLASSIFY LINE, and it is worth stating because the obvious reading is that the
 *   model produced a short draft: it reads sms=NO, so the TEXT was not degenerate when the primary
 *   returned. It was degenerate after. The regen destroyed a field it was explicitly told not to
 *   touch, and this worker took it, because the only condition on the swap was regenResult.ok —
 *   which reports that the HTTP call succeeded and nothing at all about the content.
 *
 *   So the safety net was able to replace a good draft with a worse one and log success while doing
 *   it. That is a more serious defect than the phone ask it was repairing.
 *
 *   THE GUARD. regenGuard compares the revision against the draft it would replace and rejects it
 *   unless it is plausibly the same message: no field that carried something comes back empty
 *   (subject included), no substantive field collapses below an absolute floor, and no substantive
 *   field loses most of its length. The tolerances are asymmetric on purpose — a FLAGGED field is
 *   expected to lose a sentence and may lose more; an UNFLAGGED field was supposed to come back
 *   identical, so a large loss there is evidence the model rewrote what it was told to preserve.
 *   On the incident the SMS went to 39 characters and the email kept roughly a seventh of itself,
 *   and the two tests catch the two fields independently.
 *
 *   NO SECOND CLASSIFIER CALL. The path has already spent two model calls, and this failure is a
 *   SHAPE rather than a judgement — the fields are present and they are empty. Length against the
 *   draft being replaced answers it with no extra round trip and no extra latency.
 *
 *   FAILS TOWARD THE PRIMARY. A rejected regen keeps the primary, which may still carry the phone
 *   ask. That is a small defect where an empty draft is a total one, and the trade is deliberate.
 *   The single case that passes unchecked is a primary that does not parse, because then there is
 *   no baseline and rejecting would discard the only repair available.
 *
 *   OBSERVABILITY. New REGEN REJECTED line naming the field and the measurement that failed, and
 *   FINAL now carries regenRejected — without it, a rejected regen and a regen that never ran look
 *   identical in the log, which is the exact complaint the v7.48 note in this file records about
 *   an earlier skip. The rejected case is also excluded from the clean-generation cache gate,
 *   which flagged already implied; the term is spelled out so the intent survives an edit to
 *   either flag.
 *
 *   ALSO SEEN IN THE SAME TRACE AND NOT ACTED ON: the PREFILTER shadow flagged the SMS as a phone
 *   ask and was right, while the model classifier that actually gates the regen said no. The
 *   prefilter observes only, so it changed nothing here, and gating on it is a separate decision
 *   with its own evidence bar (v7.48: ship it once wouldSkip is common AND miss has stayed at zero).
 *   Recorded rather than folded in.
 *
 *   VERIFIED: new regen-guard.test.js, 45 assertions, EXECUTING the shipped regenGuard. The two
 *   gutted drafts are verbatim from the extension's [LP RAW DIAG] on request 7e0d2e6b; the primary
 *   is reconstructed at ordinary length, because this worker logs character counts and never the
 *   text (the v9.7.489 posture) so the real primary is not recoverable. The rejection is asserted
 *   from each field independently, and the FALSE-POSITIVE direction is covered at least as heavily
 *   as the true one -- an identical regen, a whitespace reflow, a slightly longer rewording, and
 *   the one-sentence repair the regen exists to produce are all accepted, because rejecting a good
 *   revision means shipping the phone ask. The wiring is asserted too: the swap reads the verdict
 *   rather than regenResult.ok, the rejection is logged with its reason, FINAL distinguishes a
 *   rejected regen from one that never ran, and the rejected case is out of the cache gate.
 *   NON-VACUITY is behavioural and in-build, each paired with a control: removing the ratio test
 *   re-accepts the gutted email, removing the floor re-accepts a greeting-length SMS, and
 *   collapsing the two bars into one re-accepts an unflagged field that lost half of itself.
 *   101 suites green across the repo (4,711 assertions, +45). No behaviour changes on any path
 *   where the regen returns a comparable draft.
 *
 *   ONE ASSERTION OF MINE WAS WRONG AND THE CODE WAS RIGHT: I expected the incident to be reported
 *   by the ratio test, and the SMS is 39 characters, so the absolute FLOOR fires first. That is the
 *   better order and the more legible message, so the assertion was corrected to what the guard
 *   does rather than the other way round.
 *
 *   Builds on v7.74.
 */
/**
 * v7.74: THE TALLY COULD NOT READ ITS OWN OLDER ROWS. Two counter repairs, no behaviour change.
 *   Nothing about a generation, a rejection or a repair moves in this build — only the summary the
 *   export prints on top of rows that were already correct.
 *
 *   (1) emailShapes MISCOUNTED EVERY PRE-v7.73 ROW, and this is the second wrong summary over the
 *   same counter in two days. v7.73 added shape.emailIsJson and keyed the double-encoded /
 *   abandoned split on it. That is right for rows v7.73 writes and silently wrong for every row
 *   v7.72 had already put in KV: those have no such field, so `r.shape.emailIsJson` read undefined
 *   and they fell through into "abandoned". On 9/8 that reported 3 double-encoded / 6 abandoned
 *   where the nine raw rows plainly say SEVEN and TWO — the four pre-deploy rows whose samples
 *   begin '{"subject":' are double-encoded and were filed as abandonments.
 *   Replaced with a ladder, most authoritative first: outcome 'repaired' (definitionally the
 *   double-encoded shape); then shape.emailIsJson, tested for PRESENCE so a legitimate 0 is a
 *   verdict rather than a fall-through; then the sample's first character, which a pre-v7.73 row
 *   still carries and which begins '{' exactly when the field was an envelope. A row that answers
 *   to none of these is reported as `unclassified` rather than guessed into a bucket — the same
 *   instinct as the -1 used for absent lengths, which I had and did not apply here.
 *
 *   (2) byFinish SPLIT ONE VALUE ACROSS TWO BUCKETS. The reject path records openaiFin ('stop');
 *   the v7.73 repair path recorded the normalised finishReason ('STOP'). 9/8 came back as
 *   {STOP: 3, stop: 6} for what is one finish reason on nine rows. Both paths now record the raw
 *   upstream value, and the tally lowercases — so rows already in KV fold correctly and no backfill
 *   is needed to make an old row readable.
 *
 *   THE GENERAL LESSON, worth more than either fix: an export is read across builds, so a summary
 *   must be written for the rows that EXIST rather than for the rows this version writes. Both
 *   defects were mine, both were invisible in the aggregate and obvious in the raw rows, and the
 *   raw rows were right every time.
 *
 *   VERIFIED against Gil's actual 9/8 export: the nine real rows, seeded verbatim, now tally
 *   7 double-encoded / 2 abandoned / 0 unclassified and byFinish {stop: 9}. Under v7.73 the same
 *   nine produce 3/6 and {STOP: 3, stop: 6}.
 *
 *   Builds on v7.73.
 */
/**
 * v7.73: STOP THROWING AWAY GOOD EMAILS — AND COUNT THEM ON THE AXIS THAT ACTUALLY SEPARATES THEM.
 *   Both changes come straight out of the first afternoon of v7.72 data, and one of them corrects
 *   v7.72's own summary.
 *
 *   WHAT THE DATA SAID. Five rejections on 9/8, every one gpt-5.6-luna / finish=stop / primary /
 *   attempt 0 / effort low. FOUR of the five carried 410-468 characters in the email field that
 *   parse as a complete JSON object with a subject inside — a finished email, wrapped one layer too
 *   deep. Three of those four ALSO filled the top-level subject correctly, so the model wrote the
 *   subject twice. Only the fifth was a real abandonment: a 7-character email field holding the
 *   literal word "subject", with a good 24-character subject sitting correctly at top level.
 *
 *   (1) THE UNWRAP. We were discarding a verified-good email and paying for a second model call —
 *   3.7 to 5.7 seconds and a full generation, for output we already had. repairNestedEmail lifts
 *   the body out and rebuilds "Subject: <subject>\n\n<body>", the shape a clean generation
 *   produces, so a repaired draft is indistinguishable downstream. THE INNER BODY KEY IS NOT
 *   GUESSED: samples truncate at 40 chars so that name was never visible, and it takes the longest
 *   non-subject string value after preferring the conventional names. STRICTLY A RESCUE — it runs
 *   only on text already about to be rejected, returns null unless the rebuilt body passes the
 *   ordinary degenerate check on its own, and leaves the original text untouched when it returns
 *   null. It cannot turn a passing response into a failing one, and it cannot admit a body the
 *   guard would otherwise refuse.
 *
 *   A RESCUED GENERATION IS STILL A DEFECT AND IS STILL RECORDED. It now succeeds, so it would have
 *   vanished from the failure path that recorded it yesterday — and seeing whether the model's
 *   behaviour changes was the entire reason for measuring. Same key prefix, same export,
 *   distinguished by `outcome`: rejected or repaired.
 *
 *   (2) THE TALLY WAS SPLIT ON THE WRONG AXIS, and that was mine. v7.72 reported
 *   emailRejections{subjectPresent, subjectEmptyOrAbsent}. One afternoon of real rows showed it
 *   separates nothing: the 16:35 row was double-encoded WITHOUT a top-level subject, and the 13:15
 *   abandonment HAD one, so the summary filed each under the other's heading. The question that
 *   does separate them is whether the email field is itself a JSON envelope. The shape now records
 *   emailIsJson and the export reports emailShapes{doubleEncoded, abandoned} plus byOutcome. The
 *   raw rows were always right; only my summary of them was not.
 *
 *   EXPECT THE NUMBERS TO MOVE, and it is the fix working rather than the defect stopping: fallback
 *   hits and PRIMARY failures should fall by roughly the double-encoded share (4 of 5 in the only
 *   sample there is), while the /degenerate count stays about the same with most rows now reading
 *   outcome=repaired. If the count itself falls, that is the model changing, not us.
 *
 *   Builds on v7.72.
 */
/**
 * v7.72: A DAY OF DEGENERATE-EMAIL SAMPLES, ON DEMAND — AND THE ONE FACT THE LOG LINE NEVER CARRIED.
 *
 *   THE FAILURE. 27 PRIMARY rejections on 9/7 (4,770 requests), 4 on 9/6, 10 on 8/29 — every one
 *   "Degenerate field content" and, on every sample read so far, on the EMAIL field. The three
 *   captured in the 9/7 log export were [email: "subject"], [email: "subject_placeholder"] and
 *   [email: "{"subject":"Your 2022 RAV4 appraisal opt…"]. All recover on fallback, so nothing has
 *   ever reached a customer; the cost is a wasted primary call (3-7s) and a second model call, and
 *   it is most of why "with regen" latency sits near 10s against a 188ms median.
 *
 *   WHAT COULD NOT BE ANSWERED, and it is the whole reason this build exists. The failure line has
 *   named the offending field since v7.47. It has never said what the REST of the envelope held.
 *   So when email came back as the word "subject", nobody could tell whether the subject field had
 *   a real subject in it (the model collapsed two fields into one) or was empty too (the model
 *   abandoned the envelope). Those are different defects with different fixes. Guessing between
 *   them is exactly the move this project keeps paying for, so this build measures instead.
 *
 *   hasDegenerateField now returns the envelope SHAPE alongside the field and sample: the length of
 *   sms, email, subject and voicemail, the length of the email body with the Subject: line stripped,
 *   and the envelope's own top-level key names. LENGTHS, NEVER CONTENT — the v9.7.489 posture, the
 *   same shape as sysChars/userChars. -1 distinguishes "absent or not a string" from 0 "present but
 *   empty", which is itself a finding.
 *
 *   PERSISTED, BECAUSE WORKER LOGS ARE NOT. This rejection has existed only as a console line, and
 *   Cloudflare does not retain those — so "get me a day of these" has meant catching them live. One
 *   record per rejection now lands in KV beside the feedback rows (14-day TTL) and
 *   GET /degenerate?date=YYYY-MM-DD returns the day, with tallies by field, model and finish reason
 *   plus emailRejections{subjectPresent, subjectEmptyOrAbsent} — the collapse question, answered
 *   directly at the top of the response.
 *
 *   NOTHING ABOUT A GENERATION CHANGES. The record is written AFTER the failure is already decided,
 *   inside its own try, on ctx.waitUntil. A KV problem cannot slow, alter or break a draft that is
 *   already on its way to the next tier. The error STRING is unchanged, so anything keyed on
 *   "Degenerate field content" still matches verbatim.
 *
 *   DIRECTOR-GATED. /degenerate carries draft-field samples and prompt sizes, so it sits behind the
 *   same door as /feedback/drafts rather than beside the unauthenticated /valuefact.
 *
 *   WHAT IS STORED, PLAINLY: lengths, model, tier, finish reason, latency, prompt sizes, reasoning
 *   effort, attempt index, and the SAME 40-char sample the log line has carried since v7.47 — a
 *   value the guard has already judged degenerate, so in practice "subject", "{", or a JSON
 *   fragment. If a sample ever turns out to be real prose, that IS the finding, and it is 40
 *   characters of our own draft rather than of customer input. No prompt text, no lead data, no
 *   contact data.
 *
 *   NOT FIXED HERE. The cause is still unknown and is not guessed at. This build makes one day of
 *   traffic answer the question.
 *
 *   Builds on v7.71.
 */
/**
 * v7.71: THE PAIRS EXPORT COULD NOT NAME THE GENERATION IT WAS DESCRIBING. One field, added to
 *   one projection, and the write side already had it.
 *
 *   FOUND WHILE CHASING AN EMPTY DRAFT. Gil's 9/5 export carries a Community Kia Baytown row
 *   (Facebook, lead 2078978836, 8:26 AM) whose sms, email and voicemail are ALL EMPTY with
 *   prior:null — the v9.7.604 signature, which that build recorded as one agent at one store and
 *   left alone because the cause was unknown. It has now recurred on a different agent at a
 *   different store, so the one-agent locus is gone and the question "what did the model actually
 *   return for that generation" became the one worth being able to ask.
 *
 *   POST /feedback has persisted workerRequestId since v7.58 — the extension sends it (v9.7.564),
 *   it is an opaque proxy-minted UUID carrying no lead or customer data, and it sits inside the
 *   v9.7.489 posture without argument. GET /feedback/drafts, which is what every consumer of the
 *   pairs export reads, listed the row's other fields and silently dropped this one. So the data
 *   was in KV the whole time and unreachable from the artefact Gil actually works from.
 *
 *   WHAT IT JOINS TO, STATED PLAINLY, BECAUSE IT IS NOT KV. Nothing is stored under the requestId.
 *   It is the correlation id in this worker's own console lines — START, PRIMARY OK, CLASSIFY,
 *   REGEN, FINAL, SAFE_FALLBACK — so an export row now names the log trace for its generation.
 *   That is a real join and it is the one v7.58's header describes ("a generation's requestId
 *   existed only in these logs, so a /feedback row could never be tied"). It is ALSO only as good
 *   as log retention: live tailing will not reach this morning's 8:26 row, so this makes the NEXT
 *   empty draft diagnosable rather than the last one.
 *
 *   ADDITIVE AND SHAPE-PRESERVING. Conditionally spread, matching the write side line for line: a
 *   row without the key is byte-identical to what it is today, so every existing consumer behaves
 *   identically and `workerRequestId` present is itself the marker that a row is correlatable.
 *
 *   NEW SUITE feedback-export-join.test.js drives the SHIPPED handler — the worker is loaded as a
 *   module and GET /feedback/drafts is called against a stubbed KV, not regex'd — because a regex
 *   over source text is what stayed green through two live outages (see worker-smoke.test.js).
 *
 *   NOT CHANGED, AND DELIBERATELY: the empty-draft cause itself is still unknown and is not
 *   guessed at here. The reporter's Rejected Sessions table also does not carry the id; that is a
 *   reporter change and was not asked for.
 *
 *   Builds on v7.70.
 */
/**
 * v7.70: NOTHING COULD ASK HOW OLD A STORE'S INCENTIVE DATA WAS, AND THE META BLOB LIED ABOUT
 *   WHICH STORES EXISTED. Two small repairs to the same gap, both found on 9/4 chasing a Kia
 *   upload that had been correct all along.
 *   (1) GET /valuefact dropped `generated` from its projection. The POST writes it onto every
 *       blob; the read threw it away. So freshness was unanswerable from outside the worker —
 *       the Data Tool's table only shows the file you just dropped, the dashboard has no
 *       incentive view, and a survey script written to answer "is Kia in there?" printed "—"
 *       for every store's date. Now returned, alongside storedCount so the expiry kill-switch
 *       is visible (0 of 110 live reads differently from an empty upload). Additive: `count`
 *       and `incentives` are unchanged, so every existing client behaves identically.
 *   (2) POST rebuilt `valuefact:meta` / `inventory:meta` from the payload alone, so a
 *       single-store upload left the blob claiming the system knew about one store. Per-dealer
 *       keys were always correct — a Kia-only upload cannot touch Toyota's, verified against
 *       live KV — and nothing reads meta today, so this cost nothing. It is still the only
 *       record of what has been published, and it was wrong in the way that would mislead the
 *       first person to open it. Merged now, each store carrying its own generated date.
 *   Builds on v7.69.
 *
 * v7.69: A DIAGNOSTIC THAT CONTRADICTED THE ROW IT WAS WRITTEN BESIDE — AND IT COST A WRONG ANSWER.
 *   Log-line only. No stored field, no response, no behaviour change of any kind.
 *
 *   FOUND IN THE 8/28 WORKER LOGS, which read 129 of 216 comprehension probes as
 *     [COMMIT-COMP] day-lock | NO-CUSTOMER-TEXT | ... |
 *       probe:FAILED the customer has never written anything on this lead
 *
 *   Nothing failed. A customer who has never written a word cannot have named a day or mentioned
 *   an off-brand make, so the absence IS a determinate "none". Every layer already knew this:
 *   extension v9.7.574 returns it WITHOUT spending an API call and marks it vacuous:true with
 *   delta NO-CUSTOMER-TEXT; proxy v7.66 stores `vacuous` — thirty lines above this very log
 *   statement, and with a comment saying it exists so the split can be made STRUCTURALLY rather
 *   than by pattern-matching probeFailReason prose; reporter v1.19 gives it its own tile and keeps
 *   it out of the agreement rate.
 *
 *   This line looked at probeOk alone. A vacuous row carries probeOk:false, so the single most
 *   common state of an ordinary BDC day printed as breakage in the one artefact a human reads.
 *
 *   IT COST EXACTLY WHAT A MISLABEL COSTS. Reading these logs I reported to Gil that ~60% of
 *   probes were failing and proposed adding a precondition to skip them. There was nothing to
 *   skip — v9.7.574 had already done the work, and no network call was ever being spent. The
 *   recommendation was wrong and the log is why. Correcting the label is the whole fix.
 *
 *   The four states are now named distinctly: unreported (pre-v9.7.564) / ok / n/a — nothing to
 *   read (with the detector's own reason, which differs per detector) / FAILED (with its reason).
 *   A genuine failure still reads FAILED; vacuous must never swallow one, and probeOk:true still
 *   wins over a stray vacuous flag. Covered by tests/probe-label.test.js, which slices this
 *   template expression out of the shipped file and evaluates it rather than restating it —
 *   10 assertions, and v7.68 fails 4 of them by name.
 *
 *   NO EXTENSION CHANGE. The extension was correct; it stays at v9.7.603.
 *   Builds on v7.68.
 */
/**
 ...
 *
 *   (v7.78) MOVED DOWN ONE RUNG for GPT-6 Luna (Gil, 9/23). Each model keeps its own settings;
 *   the tier timeouts stay where the 24000ms budget needs them.
 *   primary    — gpt-6-luna     (12s timeout; no production latency yet — /perf will show it)
 *   fallback   — gpt-5.6-luna   (8s timeout — the budget slot; its P99 as primary was ~9.6s)
 *   emergency  — gpt-5.4-nano   (cap 8s, its own figure; the budget clamps it to 3.7s in the worst
 *                                case). NOTE: all three tiers are now reasoning models — the
 *                                cross-family, non-reasoning backstop v7.39 restored is gone again.
...
const TIER_EFFORTS = {
  // (v7.78) GPT-6 Luna's model page: "reasoning.effort supports none, low, medium (default), high,
  // xhigh, and max" — the same six as 5.6 Luna.
  ...
  // (v7.78) Not in the cascade any more. Kept so that putting it back is a one-line change to
  // MODEL_CASCADE and nothing else.
...
// (v7.78) DOWN ONE RUNG (Gil, 9/23): GPT-6 Luna primary, 5.6 Luna fallback, 5.4-nano emergency.
// Each model row moved with its own settings (tokens, temperature, gpt5, cacheBreakpoints), with
// one exception: 5.6 Luna takes the FALLBACK SLOT's 8000ms rather than its own 12000ms. The
// fallback slot cannot exceed ~9200ms or, after a primary timeout, the emergency tier is no longer
// admitted (24000 - 12000 - 9200 = 2800 = MIN_TIMEOUT_MS + TIMEOUT_SLACK_MS). The worst case is
// unchanged: 12000 + 8000 + 3700 = 23700ms, all three tiers attempted.
//   gpt-6-luna    cacheBreakpoints: true — GPT-6 takes prompt_cache_options.ttl '30m' in place of
//                 prompt_cache_retention and bills cache writes at 1.25x, the 5.6 explicit-cache
//                 model. If it rejected the breakpoint, that is a fast 400, which is not fatal:
//                 5.6 Luna answers, and the FAIL line names the status.
//   gpt-5.6-luna  cacheBreakpoints: true — how it has run as primary since v7.44. Setting it false
//                 would send it the pre-5.6 retention field it deprecated.
//   gpt-5.4-nano  tokens 3500, not the old emergency tier's 2000: this is a reasoning model and
//                 reasoning tokens count against max_completion_tokens. Timeout cap 8000, its own
//                 figure: the last tier's cap only matters when the tiers above failed FAST (the
//                 7/31 17:06 shape, all in <250ms); after timeouts the budget clamps it to 3700.
// (v7.81) The fixed sentence the extension's SMS refine system prompt carries (buildSystemPromptSmsRefine,
// v9.7.672 onward). A refine call with no explicit effort takes the tier's draftEffort like a full draft.
const SMS_REFINE_MARKER = 'An email to this customer has ALREADY been written and is going out.';
// (v7.79) draftEffort: the effort a tier uses for a FULL DRAFT whose caller asked for none (see the
// tier loop for exactly which requests that is). Set on gpt-6-luna only: it is the one model the
// none-vs-low test measured. Absent on a row = REASONING_EFFORT, exactly as before.
const MODEL_CASCADE = [
  { model: 'gpt-6-luna',              tokens: 3500, timeout: 12000, temperature: null, tier: 'primary',   gpt5: true,  cacheBreakpoints: true,  draftEffort: 'none' },
  { model: 'gpt-5.6-luna',            tokens: 3500, timeout:  8000, temperature: null, tier: 'fallback',  gpt5: true,  cacheBreakpoints: true  },
  { model: 'gpt-5.4-nano-2026-03-17', tokens: 3500, timeout:  8000, temperature: null, tier: 'emergency', gpt5: true,  cacheBreakpoints: false },
];
...
// (v7.77) NEUTRAL, BECAUSE AN OLDER EXTENSION MAY STILL SHOW IT. Extensions since v9.7.379 refuse to
// render a _fallback envelope and v9.7.706 shows the agent a failure notice instead; this text is the
// backstop for anything older. The v7.76 wording pushed an appointment ("would today or tomorrow work
// better to come in?") on every lead -- opted-out, exiting, sold or remote alike -- and carried
// unfilled [Agent] / [Store] placeholders one paste from a customer. It now asks for nothing, names
// no one and has no brackets. Same three fields, so an older client still parses it.
const SAFE_FALLBACK_TEXT = JSON.stringify({
  sms:       "Thanks for reaching out. I'm pulling your information together and will follow up shortly.",
  email:     "Subject: Following up on your inquiry\n\nHi,\n\nThanks for reaching out. I'm pulling your information together and will follow up shortly.\n\nThank you",
  voicemail: "Hi, thanks for reaching out. I'm pulling your information together and will follow up shortly.",
});
...
// (v7.77) THE LICENSE KEYS, BY PREFIX, PAGED. Every writer in this file mints licenses as 'LP-' +
// 8 characters (/provision-license), so the list is the 'LP-' prefix, read to the end with the
// cursor. KV lists keys in UTF-8 byte order, and every telemetry prefix (commit: degen: feedback:
// invite: perf:) is lowercase, so ALL uppercase keys sort before ALL telemetry: a second, cheap
// list-only pass reads from the start of the namespace and stops at the first lowercase key, which
// finds any license written in an OLDER uppercase format without ever touching telemetry. One more
// prefix read finds stray lowercase 'lp-' duplicates (the pre-v7.37 validate-license backfill wrote
// the record under the key exactly as typed). Both are returned flagged, never dropped silently.
async function _listLicenseKeys(env) {
  const out = { keys: [], other: [], lowercase: [], counts: { lp: 0, other: 0, lowercase: 0 }, pages: 0 };
  const listAll = async (opts, onKey) => {
    let cursor, stop = false;
    do {
      const page = await env.LEADPRO_LICENSES.list({ ...opts, limit: 1000, ...(cursor ? { cursor } : {}) });
      out.pages++;
      for (const k of page.keys) { if (onKey(k) === false) { stop = true; break; } }
      cursor = (stop || page.list_complete) ? null : page.cursor;
    } while (cursor);
  };
  await listAll({ prefix: 'LP-' }, k => { out.keys.push({ name: k.name, format: 'LP-' }); out.counts.lp++; });
  await listAll({}, k => {
    if (k.name.charCodeAt(0) >= 0x61) return false;            // 'a' and above: telemetry. Stop.
    if (k.name.startsWith('LP-')) return true;
    out.keys.push({ name: k.name, format: 'other' }); out.other.push(k.name); out.counts.other++;
    return true;
  });
  await listAll({ prefix: 'lp-' }, k => {
    out.keys.push({ name: k.name, format: 'lowercase' }); out.lowercase.push(k.name); out.counts.lowercase++;
  });
  return out;
}

// (v7.77) CENTRAL-DAY FEEDBACK READS. /feedback/drafts and /degenerate have answered date= as a
// Central calendar day since v7.29/v7.72; summary and range answered it as a UTC day, so a row
// written at 9:30 PM Central counted toward the NEXT day there and toward the right day everywhere
// else, including the reporter's daily email (Central since reporter v1.5/v1.7). Keys carry a UTC
// server timestamp, so a Central day spans two UTC-date prefixes; each row is placed by its own
// ts (the same field /feedback/drafts filters on), falling back to the key's server timestamp for a
// row that carries none -- the reporter keeps such rows too, so they are not dropped here.
function _centralDateOfMs(ms) {
...
async function _readFeedbackByCentralDay(env, dates) {
  const want = new Set(dates);
  const prefixes = new Set();
  for (const d of dates) for (const p of centralDayUtcWindow(d).utcPrefixes) prefixes.add(p);
  const byDate = {};
  ...
  await Promise.all([...prefixes].map(async (pfx) => {
    ...
    do {
      ...
      vals.forEach((val, i) => {
        if (!val) return;
        let e; try { e = JSON.parse(val); } catch { return; }
        let t = Date.parse(e && e.ts);
        if (!(t > 0)) t = Date.parse(page.keys[i].name.slice('feedback:'.length, 'feedback:'.length + 24));
        if (!(t > 0)) return;
        const cd = _centralDateOfMs(t);
        if (want.has(cd)) byDate[cd].push(e);
      });
...
function _normaliseAbandoned(e) {
  // (v7.52) the same retroactive rewrite aggregate() applies: an EMPTY meta is positive evidence no
  // draft was ever produced, so a v9.7.540 'abandoned' row is really 'incomplete'.
  return (e && e.rating === 'abandoned' && !(e.meta && Object.keys(e.meta).length))
...
export default {
  async fetch(request, env, ctx) {
    ...
    // (v7.72) /degenerate joins the director-gated reads. It carries draft-field samples and
    // prompt sizes, so it sits behind the same door as /feedback/drafts rather than beside the
    // unauthenticated /valuefact.
    // (v7.77) /perf joins them: request ids, sizes and timings, no content, but it is the fleet's
    // traffic shape and belongs behind the same door.
    if (request.method === 'GET' && /\/(?:feedback\/(?:summary|range|drafts)|degenerate|perf)$/.test(url.pathname)) {
    ...
    if (request.method === 'GET' && url.pathname.endsWith('/feedback/range')) {
      ...
      // (v7.77) CENTRAL DAYS. The day list is built by date-string arithmetic (not setDate on a
      // Date object, which is UTC-in-the-Worker only by accident of the runtime's zone), and every
      // row is placed on its own Central day. Each UTC-date prefix is read ONCE for the whole
      // range, so a Central day spanning two prefixes does not double the reads.
      if (!/^\d{4}-\d{2}-\d{2}$/.test(fromParam) || !/^\d{4}-\d{2}-\d{2}$/.test(toParam)) {
        return corsResponse('{"error":"from and to must be YYYY-MM-DD (Central days)"}', 400);
      }
      const dates = [];
      for (let cur = fromParam; cur <= toParam && dates.length < 90; cur = _shiftDateStr(cur, 1)) dates.push(cur);

      const allEntries = [];
      const byDay = {};
      const _byDate = await _readFeedbackByCentralDay(env, dates);
      ...
      dates.forEach((date) => {
        ...
        const dayEntries = _byDate[date].map(_normaliseAbandoned);
        for (const e of dayEntries) allEntries.push(e);
      ...
      return corsResponse(JSON.stringify({
        from: fromParam, to: toParam, dates, tz: 'America/Chicago',
        ...summary,
        byDay,
      }), 200);
    }
    ...
    if (request.method === 'GET' && url.pathname.endsWith('/feedback/summary')) {
      // (v7.77) date= is a CENTRAL day, and the default is Central today (was UTC today, which
      // after 7 PM Central was already tomorrow). Same basis as /feedback/drafts and the reporter.
      const dateParam = url.searchParams.get('date') || centralTodayStr();
      if (!/^\d{4}-\d{2}-\d{2}$/.test(dateParam)) return corsResponse('{"error":"date must be YYYY-MM-DD (Central day)"}', 400);
      let entries;
      try {
        entries = (await _readFeedbackByCentralDay(env, [dateParam]))[dateParam];
      } catch(e) {
      ...
      const chipFreq = {};
      const byScenario = {};
      // (v7.77) byScenario gets what every other bucket already had. (a) e.signal, as v7.51 gave
      // byStore/byPersona/byLeadSource/byFlag -- without it every scenario's explicitDown was 0 and
      // every rejection counted as implicit. (b) the v7.52 abandoned->incomplete rewrite, which
      // aggregate() applies internally for the top line; this loop read the raw rows, so a pre-541
      // 'abandoned' row sat in a scenario's abandoned column and the top line's incomplete column,
      // and the buckets could not reconcile with the totals beside them.
      for (const e of entries.map(_normaliseAbandoned)) {
        (e.chipsUsed || []).forEach(c => { chipFreq[c] = (chipFreq[c] || 0) + 1; });
        const sc = e.meta?.scenario || 'unknown';
        if (!byScenario[sc]) byScenario[sc] = _newBucket();
        _tallyBucket(byScenario[sc], e.rating, e.signal);
      }
      return corsResponse(JSON.stringify({
        date: dateParam, tz: 'America/Chicago',
        ...summary,
        chipFreq,
            ...
            for (const val of vals) {
              ...
              try {
                ...
                rows.push({
                  ...
                  // (v7.71) THE FIELD THE WRITE SIDE HAS KEPT SINCE v7.58 AND THIS PROJECTION
                  // THREW AWAY. POST /feedback persists workerRequestId; every consumer of the
                  // pairs export reads it from here, so a row reached the export with no way to
                  // tie it to the generation it rates. Spread conditionally, exactly as the write
                  // side does: a pre-v9.7.564 row has no such key and keeps the shape it has
                  // today, so `workerRequestId` present is itself the marker that a row is
                  // correlatable rather than a field that is sometimes empty.
                  ...(e.workerRequestId ? { workerRequestId: e.workerRequestId } : {}),
                  // (v7.76) AND THE BUILD, FOR THE SAME REASON AND ON THE SAME TERMS. Persisting
                  // a field the export throws away is the exact defect v7.71 was written for, so
                  // the read path gains it in the same commit as the write path rather than a
                  // release later. Conditional, so a row written before v9.7.684 comes back
                  // byte-identical to what it does today.
                  ...(e.extensionVersion ? { extensionVersion: e.extensionVersion } : {}),
                  meta:       e.meta || {},
                  drafts:     e.drafts,
                });
              } catch {}
    ...
    // (v7.77) GET /perf?date=YYYY-MM-DD — one Central day of perf: rows plus the tallies the M4
    // cost question, the prefilter decision and the edge-cache decision are waiting on.
    if (request.method === 'GET' && url.pathname.endsWith('/perf')) {
      const dateParam = url.searchParams.get('date') || centralTodayStr();
      if (!/^\d{4}-\d{2}-\d{2}$/.test(dateParam)) return corsResponse('{"error":"date must be YYYY-MM-DD (Central day)"}', 400);
      const { startMs, endMs, utcPrefixes } = centralDayUtcWindow(dateParam);
      const rows = [];
      try {
        for (const pfx of utcPrefixes) {
          let cursor;
          do {
            const page = await env.LEADPRO_LICENSES.list({ prefix: 'perf:' + pfx, limit: 1000, ...(cursor ? { cursor } : {}) });
            const vals = await Promise.all(page.keys.map(key => env.LEADPRO_LICENSES.get(key.name)));
            for (const val of vals) {
              if (!val) continue;
              try {
                ...
                if (t >= startMs && t < endMs) rows.push(r);
              } catch {}
            }
            cursor = page.list_complete ? null : page.cursor;
          } while (cursor);
      ...
      rows.sort((a, b) => (a.ts < b.ts ? -1 : 1));
      return corsResponse(JSON.stringify({ date: dateParam, tz: 'America/Chicago', count: rows.length,
        tallies: _perfTallies(rows), rows }), 200);
    }

    if (request.method === 'GET' && url.pathname.endsWith('/degenerate')) {
      const dateParam = url.searchParams.get('date') || centralTodayStr();
      const { startMs, endMs, utcPrefixes } = centralDayUtcWindow(dateParam);
      const rows = [];
      try {
        for (const pfx of utcPrefixes) {
          let cursor;
          do {
            const page = await env.LEADPRO_LICENSES.list({ prefix: 'degen:' + pfx, limit: 1000, ...(cursor ? { cursor } : {}) });
            const vals = await Promise.all(page.keys.map(key => env.LEADPRO_LICENSES.get(key.name)));
            for (const val of vals) {
              if (!val) continue;
              try {
      ...
      rows.sort((a, b) => (a.ts < b.ts ? 1 : -1));
      // Tallies so the shape of the day is answerable without reading every row. subjectPresent
      // is the one the whole build exists to answer: on a rejected email, did a real subject come
      // back alongside it?
      // (v7.73) SPLIT ON THE RIGHT AXIS. v7.72 counted "was there a top-level subject", and the
      // first afternoon of real rows proved that separates nothing: a double-encoded email arrived
      // WITHOUT a top-level subject and a genuine abandonment arrived WITH one. What distinguishes
      // them is whether the email field was itself a JSON envelope, which the shape now records.
      // (v7.74) A TALLY THAT READS ONLY ITS OWN BUILD'S ROWS IS A TALLY THAT LIES, and this is the
      // SECOND wrong summary over the same counter in two days. v7.73 introduced shape.emailIsJson
      // and keyed the split on it — correct for rows v7.73 writes, and silently wrong for every row
      // v7.72 already had in KV. On 9/8 that put the four pre-deploy double-encoded rows into the
      // abandoned bucket and reported 3/6 where the raw rows plainly say 7/2. The rows were right
      // both times; only my reading of them was not.
      // The fix is a LADDER, most authoritative first, and a row that answers to none of it is
      // counted as unclassified rather than guessed into a bucket:
      //   1. outcome 'repaired'  — definitionally the double-encoded shape; it is all the unwrap
      //                            rescues, and its sample is the literal '(unwrapped)'.
      //   2. shape.emailIsJson   — present from v7.73 on. Checked for PRESENCE, not truthiness, so
      //                            a legitimate 0 is a verdict and not a fall-through.
      //   3. the sample's first character — a pre-v7.73 row still carries the field value, and it
      //                            begins '{' exactly when the field was an envelope. Derived, and
      //                            said to be derived, rather than assumed.
      // byFinish lowercases for the same reason: fold what is already stored, do not require a
      // backfill to make an old row readable.
      const byField = {}, byModel = {}, byFinish = {}, byOutcome = {};
      let doubleEncoded = 0, abandoned = 0, unclassified = 0;
      for (const r of rows) {
        byField[r.field]          = (byField[r.field]          || 0) + 1;
        byModel[r.model]          = (byModel[r.model]          || 0) + 1;
        const _fin = String(r.finish || '?').toLowerCase();
        byFinish[_fin] = (byFinish[_fin] || 0) + 1;
        byOutcome[r.outcome || 'rejected'] = (byOutcome[r.outcome || 'rejected'] || 0) + 1;
        if (r.field !== 'email') continue;
        let nested = null;
        if (r.outcome === 'repaired') nested = true;
        else if (r.shape && typeof r.shape.emailIsJson === 'number') nested = !!r.shape.emailIsJson;
        else if (typeof r.sample === 'string' && r.sample) nested = r.sample.trim().startsWith('{');
        if (nested === true) doubleEncoded++;
        else if (nested === false) abandoned++;
        else unclassified++;
      }
      return corsResponse(JSON.stringify({
        date: dateParam, tz: 'America/Chicago', count: rows.length,
        byField, byModel, byFinish,
        byOutcome,
        emailShapes: { doubleEncoded, abandoned, unclassified },
        rows,
      }), 200);
    }
    ...
    if (request.method === 'POST' && url.pathname.endsWith('/feedback')) {
      ...
      const fbValue = JSON.stringify({
        ...
        // (v7.76) WHICH BUILD WROTE THIS ROW. A version string, not customer data. Same treatment
        // as workerRequestId above: clamped, and omitted when absent so a pre-v9.7.684 extension
        // produces exactly the row it produces today. Its presence is the marker that the row can
        // be attributed at all, which is what makes "is a stale build in the field?" a filter
        // instead of a deduction from which fields happen to be missing.
        ...(fb.extensionVersion ? { extensionVersion: String(fb.extensionVersion).slice(0, 24) } : {}),
        meta:       fb.meta       || {},
        ...((fb.drafts && typeof fb.drafts === 'object') ? { drafts: fb.drafts } : {})
      });
      try {
        await env.LEADPRO_LICENSES.put(fbKey, fbValue, { expirationTtl: 60 * 60 * 24 * 90 });
        console.log(`[FEEDBACK] ${fb.rating} | ${fb.signal} | ${(fb.meta||{}).store||'?'} | chips: ${(fb.chipsUsed||[]).join(',') || 'none'}${fb.drafts ? ' | +drafts' : ''} | build: ${fb.extensionVersion || '(unstamped)'}`);
    ...
    if (request.method === 'POST' && url.pathname.endsWith('/commit-comprehension')) {
      ...
      // (v7.68) TAKES cc.id, NOT requestId. v7.67 passed `requestId`, which is declared 254 lines
      // BELOW this handler (line ~1587) in a branch /commit-comprehension returns before reaching.
      // `const` hoists but stays uninitialised, so every call threw
      //   ReferenceError: Cannot access 'requestId' before initialization
      // and EVERY /commit-comprehension POST 500'd from the moment v7.67 deployed — 93 of 93 on
      // 8/26, zero rows stored, the report reading "No comprehension verdicts recorded for this
      // date". The extension was sending correctly the whole time.
      // Same class as the v7.64 isEscalation/reqEffort crash: a name that is not in scope where it
      // is used. That one was a wrong name, this one a declaration order — and BOTH shipped past
      // suites that only checked the code was present. cc.id is the generation id off the payload
      // being validated three lines above, so it is in scope AND ties the log to the generation.
      ...
      const ccValue = JSON.stringify({
        ...
        detector:       _ccDetector(cc.detector, cc.id),
      ...
      try {
        ...
        console.log(`[COMMIT-COMP] ${CC_DETECTORS.indexOf(cc.detector) >= 0 ? cc.detector : 'verbal-commit'}`
          ...
          // (v7.69) THIS LINE CONTRADICTED THE ROW IT WAS WRITTEN BESIDE. It derived its label
          // from probeOk alone and ignored `vacuous` — the field stored 30 lines above for the
          // express purpose of splitting "nothing to read" from "probe broke" STRUCTURALLY rather
          // than by pattern-matching prose. A vacuous row carries probeOk:false, so every lead
          // where the customer has simply never written anything printed as
          // "probe:FAILED the customer has never written anything on this lead".
          // That is not a failure. Extension v9.7.574 returns it WITHOUT spending an API call,
          // stores it as delta NO-CUSTOMER-TEXT, and reporter v1.19 already gives it its own tile
          // and keeps it out of the agreement rate. Storage and report were correct; only this
          // log lied — and on the 8/28 logs it read as 129 probe failures out of 216, which is
          // the majority of a normal BDC day mislabelled as breakage in the one artefact a human
          // actually reads. Named separately now, so the log agrees with the row.
          + ` | probe:${cc.probeOk === undefined ? 'unreported (pre-v9.7.564)'
                : cc.probeOk ? 'ok'
                : cc.vacuous ? 'n/a — nothing to read (' + (cc.probeFailReason || '?') + ')'
                : 'FAILED ' + (cc.probeFailReason || '?')}`
          + ` | ${ccMeta.store || '?'}`);
        return corsResponse('{"ok":true}', 200);
      } catch (e) {
    ...
    if (request.method === 'POST' && url.pathname.endsWith('/list-licenses')) {
      ...
      // (v7.77) LIST LICENSES, AND ONLY LICENSES. See the v7.77 header. One unprefixed list() read
      // the first 1,000 keys of a namespace that also holds commit:, degen:, feedback:, invite: and
      // (from v7.77) perf: rows, parsed every non-feedback row as a "license" with one KV read
      // each, and handed invite rows -- which carry customer email addresses -- back in the list.
      const _lic = await _listLicenseKeys(env);
      const pairs = await Promise.all(
        _lic.keys.map(async k => {
          const v = await env.LEADPRO_LICENSES.get(k.name);
          if (!v) return null;
          try {
            return { licenseKey: k.name, ...JSON.parse(v), ...(k.format !== 'LP-' ? { keyFormat: k.format } : {}) };
          } catch { return null; }
        })
      );
      const records = pairs.filter(Boolean);
      console.log(`[LIST-LICENSES] LP-=${_lic.counts.lp} other-uppercase=${_lic.counts.other} lowercase-lp=${_lic.counts.lowercase}`
        + ` returned=${records.length} listPages=${_lic.pages}`);

      return corsResponse(JSON.stringify({
        ok: true, licenses: records,
        // Keys that are license-shaped but NOT the current LP- format are reported by name, so an
        // older format is visible rather than silently included or silently dropped.
        keyFormats: { lp: _lic.counts.lp, other: _lic.other, lowercase: _lic.lowercase },
      }), 200);
    }
    ...
    if (request.method === 'GET' && url.pathname.endsWith('/valuefact')) {
      ...
      if (Array.isArray(vf.incentives)) {
        ...
        // (v7.70) FRESHNESS WAS UNANSWERABLE FROM OUTSIDE. This projection returned
        // {store, count, incentives} and dropped `generated`, which the POST writes onto every
        // blob — so nothing outside the worker could ask how old a store's incentive data was.
        // On 9/4 that cost a full day: a Kia upload returned 200, the Data Tool's summary table
        // shows only the file you just dropped, the dashboard has no incentive view at all, and
        // the survey script written to answer "is Kia in there?" printed "—" for every store's
        // date because THIS LINE removed it. The data had been correct the whole time.
        // Additive only: `count` keeps its exact meaning (lines that survived filtering) and the
        // extension reads valuefact.incentives, so no existing client changes behaviour.
        // storedCount vs count makes the expiry kill-switch visible — a store whose lines have
        // all lapsed now reads 0 of 110 rather than looking like an empty upload.
        return corsResponse(JSON.stringify({ ok: true, valuefact: {
          store: vf.store,
          count: live.length,
          storedCount: vf.incentives.length,
    ...
    if (request.method === 'POST' && (url.pathname.endsWith('/inventory') || url.pathname.endsWith('/valuefact'))) {
      ...
      // (v7.70) THE META BLOB WAS REPLACED WHOLESALE AND SO IT LIED. Per-dealer keys are written
      // individually and are correct — a Kia-only upload cannot touch Toyota's record, verified
      // against live KV on 9/4 — but `meta` was rebuilt from the payload alone, so after a
      // single-store upload it claimed the system knew about one store. Nothing reads it today
      // (zero readers in the worker, the dashboard, the reporter or either extension build), so
      // this has cost nothing; it is the only record of "what has been published" and it was
      // wrong in exactly the way that would mislead the first person to open it. Merged now, with
      // each store carrying its OWN generated date and timestamp rather than the batch's, so the
      // blob answers "when was each store last published" instead of "what was in the last file".
      let meta = { updatedAt: Date.now(), pushedBy: licenseKey, stores: {} };
      try {
        const priorRaw = await env.LEADPRO_REGISTRY.get(keyPfx + 'meta');
        if (priorRaw) {
          const prior = JSON.parse(priorRaw);
          if (prior && prior.stores && typeof prior.stores === 'object') meta.stores = prior.stores;
        }
      } catch (eMeta) { /* unreadable prior meta — start clean rather than fail the upload */ }
      meta.generated = gen; // most recent publish, kept for backward compatibility
      const writes = [];
      for (const did of Object.keys(payload.stores)) {
        ...
        meta.stores[did] = { store: s.store, count: n, generated: gen, updatedAt: Date.now() };
    ...
    // (v7.77) THE BUILD THAT ASKED. Extension v9.7.706+ sends extensionVersion on /generate, from the
    // same manifest read as the feedback rows (v9.7.684). Clamped to 24 characters of version-string
    // alphabet; absent means an older extension, and every output below is then byte-identical to
    // v7.76 -- the START line gains nothing and degen: rows gain no key.
    const extVer = (typeof body.extensionVersion === 'string')
      ? body.extensionVersion.replace(/[^A-Za-z0-9._+-]/g, '').slice(0, 24) : '';
    ...
    if (edgeHit) {
      ...
      // (v7.77) A HIT is a generation the fleet asked for; without its row the HIT count /perf
      // exists to measure would always read zero. No model ran, so tier is 'edge'.
      _writePerfRow(env, ctx, _perfEdgeHit(_perf));
    ...
    _perf.requestId = requestId;
    _perf.edgeCache = noEdgeCache ? 'BYPASS' : 'MISS';
    ...
    // (v7.79) WHICH REQUESTS TAKE A TIER'S draftEffort. All three must hold:
    //   - the caller sent NO reasoningEffort (an explicit value, 'low' included, is always honoured);
    //   - the contract is 'draft' (fact probes already send 'none' themselves);
    //   - the system prompt carries the cache sentinel. Only buildSystemPrompt emits it, so this is
    //     the main draft and the voicemail button (which builds on it). The SMS refine pass and the
    //     translation call have their own short prompts, were NOT in the none-vs-low test, and stay
    //     on REASONING_EFFORT until they are measured.
    // reqEffort itself is untouched, so escalation, the recovery-tier clamp and the START line mean
    // exactly what they did.
    // (v7.81) ...or the SMS refine pass, recognised by the fixed sentence its system prompt opens on.
    const _isSmsRefine = String(systemText || '').indexOf(SMS_REFINE_MARKER) > -1;
    const _effortDefaultable = !gen.reasoningEffort && contract === RESPONSE_CONTRACT_DRAFT
      && (String(systemText || '').indexOf('\u27E6LP_CACHE_BREAKPOINT\u27E7') > -1 || _isSmsRefine);

    for (let i = 0; i < MODEL_CASCADE.length; i++) {
      const spec = MODEL_CASCADE[i];
      const _tierEffort = (_effortDefaultable && spec.draftEffort) ? spec.draftEffort : reqEffort;
      ...
      if (_tierEffort !== reqEffort) {
        console.log(`[EFFORT] ${spec.tier} ${spec.model}: no effort requested on ${_isSmsRefine ? 'the SMS refine pass' : 'a full draft'} → "${_tierEffort}" (tier draftEffort, ${_isSmsRefine ? 'v7.81' : 'v7.79'})`);
      }
      const result = await callOpenAI({
        spec, messages, maxTokens,
        authHeader, timeoutMs: timeout,
        cacheKey, reasoningEffort: _tierEffort, contract,
      });
      ...
      if (result.ok) {
        ...
        if (result.subjectLifted) {
          console.log(`[${requestId}] SUBJECT-LIFTED from=${result.subjectLifted.from} chars=${result.subjectLifted.chars}`);
        }
        logCacheHit(requestId, spec.tier, result.usage, systemText, userText, cacheKey);
        try {
          const _u = result.usage || {};
          Object.assign(_perf, { tier: spec.tier, model: spec.model, latency: result.latency,
            effort: _tierEffort, subjectLifted: !!result.subjectLifted,
            promptTokens:  (typeof _u.prompt_tokens === 'number') ? _u.prompt_tokens : null,
            cachedTokens:  (_u.prompt_tokens_details && typeof _u.prompt_tokens_details.cached_tokens === 'number') ? _u.prompt_tokens_details.cached_tokens : null,
            writtenTokens: (_u.prompt_tokens_details && typeof _u.prompt_tokens_details.cache_write_tokens === 'number') ? _u.prompt_tokens_details.cache_write_tokens : null });
        } catch (e) { /* telemetry must never break a generation */ }
        // (v7.73) A RESCUED GENERATION IS STILL A DEFECT AND MUST STILL BE COUNTED. It now
        // succeeds, so it would vanish from the failure path that recorded it yesterday — and the
        // whole point of measuring this was to see whether the model's behaviour changes. Same
        // key prefix, same export, distinguished by `outcome`.
        if (result.repaired) {
          try {
            const _rpRow = {
              ts: new Date().toISOString(), requestId, tier: spec.tier, model: spec.model,
              latency: result.latency, finish: result.finishRaw || result.finishReason || '',
              field: 'email', outcome: 'repaired', sample: '(unwrapped)',
              repair: result.repaired,
              sysChars: (systemText || '').length, userChars: (userText || '').length,
              effort: _tierEffort, attempt: i,
              ...(extVer ? { extensionVersion: extVer } : {}),
            };
            console.warn(`[${requestId}] DEGENERATE-REPAIRED ${JSON.stringify(_rpRow)}`);
            ctx.waitUntil(env.LEADPRO_LICENSES.put(
              `degen:${_rpRow.ts}:${requestId}`, JSON.stringify(_rpRow),
        ...
        // (v7.75) Why a flagged generation shipped its PRIMARY anyway. Without this, a rejected
        // regen and a regen that never ran look identical in FINAL, which is the shape of problem
        // the v7.48 note above this block was written about.
        let regenRejected  = null;
        ...
        try {
          ...
          if (phoneOnFile) {
            ...
            if (smsText.length > 20 || emailText.length > 20) {
              // (v7.77) A FAILED CLASSIFICATION IS NOT A "NO". classifyPhoneAsk used to return false on
              // a non-OK response or a throw, so a broken classifier printed sms=NO and looked exactly
              // like a clean draft -- v7.64 hid a 400 on every call that way. It now returns a state;
              // a failure is logged on its own CLASSIFY line with the status or error, sets
              // classifyFailed (which the edge-cache gate already excludes), and counts as no flag for
              // that field, so the behaviour is unchanged: the primary draft is kept.
              const _skip = Promise.resolve({ state: 'no' });
              const [smsRes, emailRes] = await Promise.all([
                smsText.length   > 20 ? classifyPhoneAsk(smsText,   authHeader) : _skip,
                emailText.length > 20 ? classifyPhoneAsk(emailText, authHeader) : _skip,
              ]);
              const smsFlag   = smsRes.state === 'yes';
              const emailFlag = emailRes.state === 'yes';
              const _cfWord = r => r.state === 'failed' ? 'FAILED' : (r.state === 'yes' ? 'YES' : 'NO');

              classifyDiag = `sms=${_cfWord(smsRes)} email=${_cfWord(emailRes)} phoneOnFile=true`;
              console.log(`[${requestId}] CLASSIFY ${classifyDiag}`);
              _perf.classifier = 'ran';
              if (smsRes.state === 'failed' || emailRes.state === 'failed') {
                classifyFailed = true;
                _perf.classifier = 'failed';
                const _cfWhy = r => r.state !== 'failed' ? 'ok' : (r.status ? 'HTTP ' + r.status : (r.error || 'unknown'));
                console.warn(`[${requestId}] CLASSIFY FAILED sms=${_cfWhy(smsRes)} email=${_cfWhy(emailRes)} (treated as no flag; keeping primary output)`);
              }
              ...
              try {
                ...
                // (v7.77) a FAILED verdict is not evidence the prefilter was right; say so in the line.
                const _mv = (flag, r) => r.state === 'failed' ? 'FAILED' : String(flag);
                console.log(`[${requestId}] PREFILTER shadow wouldSkip=${!pfSms && !pfEmail}`
                  + ` pf(sms=${pfSms} email=${pfEmail}) model(sms=${_mv(smsFlag, smsRes)} email=${_mv(emailFlag, emailRes)})`
                  + ` miss=${pfMiss}${pfMiss ? ' <-- WOULD HAVE MISSED A REAL PHONE ASK' : ''}`);
                _perf.prefilterWouldSkip = !pfSms && !pfEmail;
                _perf.prefilterMiss = pfMiss;
              } catch (pfErr) {
              ...
              if (smsFlag || emailFlag) {
                ...
                if (regenRemaining >= MIN_TIMEOUT_MS + TIMEOUT_SLACK_MS) {
                  ...
                  const regenResult = await callOpenAI({
                    ...
                    cacheKey, reasoningEffort: _tierEffort,   // (v7.79) the effort of the draft it edits
                  });

                  // (v7.75) regenResult.ok says the HTTP call succeeded. It says nothing about
                  // whether the model honoured "keep every other field identical", and on 9/11 it
                  // did not. The replacement is now conditional on still being the same message.
                  const regenVerdict = regenGuard(result.text, regenResult.text, { sms: smsFlag, email: emailFlag });
                  if (regenResult.ok && !regenVerdict.ok) {
                    regenRejected = regenVerdict.why;
                    console.warn(`[${requestId}] REGEN REJECTED ${spec.model} ${regenResult.latency}ms → ${regenVerdict.why} (keeping primary output)`);
                  } else if (regenResult.ok) {
        ...
        console.log(`[${requestId}] FINAL total=${finalTotal}ms regenerated=${regenerated} flagged=${flagged} classifyFailed=${classifyFailed}`
          + (regenRejected ? ` regenRejected="${regenRejected}"` : ''));
        ...
        _writePerfRow(env, ctx, Object.assign(_perf, { total: finalTotal, regenerated, regenRejected: !!regenRejected }));

        // (v7.75) regenRejected implies flagged, so this gate already excluded the case; the term
        // is added so the intent survives a future edit to either flag rather than relying on one
        // implying the other.
        if (i === 0 && !regenerated && !flagged && !classifyFailed && !regenRejected) {
          try {
      ...
      // (v7.72) A DAY OF SAMPLES, ON DEMAND. This rejection has been visible only as a Worker
      // console line, and Worker logs are not retained — so "get me a day of these" has meant
      // catching them live. One record per rejection now lands in KV beside the feedback rows and
      // GET /degenerate returns the day. Nothing about the generation changes: this runs AFTER the
      // failure is already decided, inside its own try, on ctx.waitUntil, so a KV problem cannot
      // slow or break a draft that is already on its way to the next tier.
      // WHAT IS STORED: lengths, the model, the finish reason, the prompt sizes, and the SAME
      // 40-char sample the log line has carried since v7.47 — a value the guard has already judged
      // degenerate, so in practice "subject", "{", or a JSON fragment. If a sample ever turns out
      // to be real prose, that IS the finding and it is 40 characters of our own draft, not of
      // customer input. No prompt text, no lead data, no contact data.
      if (result.degenerate) {
        try {
          const _dgRow = {
            ts:        new Date().toISOString(),
            requestId, tier: spec.tier, model: spec.model,
            latency:   result.latency,
            finish:    result.degenerate.finish || '',
            field:     result.degenerate.field,
            outcome:   'rejected',
            sample:    result.degenerate.sample,
            shape:     result.degenerate.shape || null,
            sysChars:  (systemText || '').length,
            userChars: (userText   || '').length,
            effort:    _tierEffort,
            attempt:   i,
            ...(extVer ? { extensionVersion: extVer } : {}),
          };
          console.warn(`[${requestId}] DEGENERATE ${JSON.stringify(_dgRow)}`);
          ctx.waitUntil(env.LEADPRO_LICENSES.put(
            `degen:${_dgRow.ts}:${requestId}`, JSON.stringify(_dgRow),
            { expirationTtl: 60 * 60 * 24 * 14 }));
        } catch (e) { /* telemetry must never break a generation */ }
      }
    ...
    _writePerfRow(env, ctx, Object.assign(_perf, { tier: 'exhausted', model: lastModel || null, total: Date.now() - startTime }));
...
async function callOpenAI({ spec, messages, maxTokens, authHeader, timeoutMs, cacheKey, reasoningEffort, contract }) {
  ...
  // (v7.78) "Luna" is now both Luna tiers: gpt-6-luna (primary) and gpt-5.6-luna (fallback).
  let _msgs = messages;
  ...
  try {
    ...
    if (!res.ok) {
      ...
      // (v7.78) A MODEL-SPECIFIC 4xx IS NOT FATAL. `fatal` stops the whole cascade, which is right
      // for a bad API key (every tier shares it) and wrong for "this project cannot use this
      // model": OpenAI answers that with 404 model_not_found (or a 403 naming the model), and the
      // NEXT tier, a different model, would work. Until v7.78 every tier was a model the project
      // had been running; a brand-new primary is exactly when this happens. Without this, a
      // gpt-6-luna that is not yet enabled on the project would put SAFE_FALLBACK on EVERY
      // request, with 5.6 Luna never tried.
      const modelSpecific = errBody?.error?.code === 'model_not_found' || /\bmodel\b/i.test(errMsg);
      const fatal   = (status === 401 || status === 403 || status === 404) && !modelSpecific;
    ...
    // (v7.73) Rescue first, judge second. The shape recorded below is taken BEFORE the repair so
    // the telemetry still describes what the model actually returned, not what we made of it.
    // `text` is a const, so the repaired output gets its OWN binding. Reassigning it threw
    // TypeError into the outer catch, which turned every rescued response into a generic tier
    // failure — all three tiers burned and the canned safe fallback served. Strictly worse than
    // not having the feature. Caught by degenerate-samples.test.js driving the real request path.
    let _repair = null, _outText = text;
    if (_contract === RESPONSE_CONTRACT_DRAFT && isLikelyJson(text) && hasDegenerateField(text)) {
      _repair = repairNestedEmail(text);
      if (_repair) _outText = _repair.text;
    }
    const degenerate = (_contract === RESPONSE_CONTRACT_DRAFT && isLikelyJson(_outText))
      ? hasDegenerateField(_outText) : null;
    if (degenerate) {
      return {
        ok: false,
        latency: Date.now() - t0,
        error: `Degenerate field content (finish=${openaiFin}) [${degenerate.field}: "${degenerate.sample}"]`,
        // (v7.72) The same facts, structured, so the call site can persist them without
        // re-parsing its own error string. The string above is unchanged — log parsing and
        // alerting keyed on "Degenerate field content" still match verbatim.
        degenerate: {
          field:  degenerate.field,
          sample: degenerate.sample,
          shape:  degenerate.shape || null,
          finish: openaiFin,
        },
      };
    }

    // (v7.79) A SUBJECT IN THE WRONG PLACE IS KEPT, NOT LOST. In the none-vs-low test, 7 of 33
    // gpt-6-luna drafts at 'none' and 2 of 33 at 'low' had no top-level "subject": the subject was the
    // email's first line ("Subject: ..." or "subject: ...") in 6 and 1, inside an email OBJECT in 0
    // and 1, and nowhere at all in 1 and 0. Neither is degenerate,
    // so nothing above touches it, and the extension strips any leading Subject: line from the body
    // and takes the subject ONLY from the top-level key — the model's subject was discarded and a
    // generic one built. Lifting it to the top-level key is all the extension needs. The email is
    // left exactly as returned.
    let _lift = null;
    if (_contract === RESPONSE_CONTRACT_DRAFT) {
      const _l = liftEmbeddedSubject(_outText);
      if (_l) { _outText = _l.text; _lift = _l.info; }
    }
    ...
    return {
      ...
      subjectLifted: _lift,
      finishReason,
      // (v7.74) The RAW upstream finish reason, alongside the normalised one. The reject path
      // records openaiFin ('stop') and the repair path was recording finishReason ('STOP'), so one
      // value arrived in the export under two spellings and byFinish split it across two buckets.
      // Both paths now record the raw value; the tally lowercases as well, so rows already written
      // fold correctly instead of needing a backfill.
      finishRaw: openaiFin,
      repaired: _repair ? _repair.info : null,
      latency: Date.now() - t0,
...
// (v7.77) DURABLE PERFORMANCE COUNTERS. Worker console lines are not retained, which is why the M4
// cost question, the prefilter miss rate and the edge-cache hit rate were all unanswerable. One small
// row per generation under perf:<ts>:<requestId>, 14-day TTL -- one KEY per row, never a shared
// counter, because KV allows one write per key per second and a counter would drop increments under
// load. Written on ctx.waitUntil inside its own try, exactly like degen: rows: it must never slow or
// break a generation. Holds ids, sizes, timings, token counts and verdict flags -- never prompt text,
// customer data or draft content (v9.7.489). env.PERF_ROWS='off' stops the writes; 'draft' limits
// them to the draft contract (the fact probes are ~40% of calls).
function _writePerfRow(env, ctx, perf) {
  try {
    const mode = String((env && env.PERF_ROWS) || 'all').toLowerCase();
    if (mode === 'off') return;
...
// The row's shape, in one place. Field order is the row's order in KV.
function _newPerfState(extVer, responseContract, systemText, userText) {
  return {
    ts: null, requestId: null, extensionVersion: extVer || null, contract: normalizeContract(responseContract),
    tier: null, model: null, latency: null, total: null,
    sysChars: systemText.length, userChars: userText.length,
    promptTokens: null, cachedTokens: null, writtenTokens: null,
    edgeCache: null, classifier: null, prefilterWouldSkip: null, prefilterMiss: null,
    regenerated: false, regenRejected: false,
    effort: null, subjectLifted: false,   // (v7.79)
    // (v7.80) A FULL draft: the draft contract AND the cache sentinel, which only buildSystemPrompt
    // emits (the main draft and the voicemail button). The SMS refine pass and translation are also
    // 'draft' contract but carry their own short prompts; v7.79's tally counted them as drafts.
    fullDraft: normalizeContract(responseContract) === RESPONSE_CONTRACT_DRAFT
...
function _perfEdgeHit(perf) {
  return Object.assign(perf, { requestId: crypto.randomUUID(), tier: 'edge', edgeCache: 'HIT', total: 0 });
}

// sysChars bands for the M4 comparison. Pre-M4 draft system prompts ran 20-25k characters; M4
// (v9.7.700) moved ~7.6k of standing rules above the cache breakpoint, putting them near 32.5k.
const PERF_M4_SYSCHARS = 30000;
function _perfTallies(rows) {
  const med = a => { if (!a.length) return null; const s = a.slice().sort((x, y) => x - y); const n = s.length;
    return n % 2 ? s[(n - 1) / 2] : (s[n / 2 - 1] + s[n / 2]) / 2; };
  const mean = a => a.length ? Math.round(a.reduce((x, y) => x + y, 0) / a.length) : null;
  const t = { calls: rows.length, byTier: {}, byContract: {}, edgeCache: { HIT: 0, MISS: 0, BYPASS: 0 },
    classifier: { ran: 0, skipped: 0, failed: 0 }, prefilter: { evaluated: 0, wouldSkip: 0, wouldSkipRate: null, miss: 0 },
    regenerated: 0, regenRejected: 0, tokensBySysCharsBand: {},
    // (v7.79) primary-tier drafts by the effort actually sent, so none-vs-low is visible on real traffic.
  ...
  for (const r of rows) {
    t.byTier[r.tier || 'unknown'] = (t.byTier[r.tier || 'unknown'] || 0) + 1;
    t.byContract[r.contract || 'unknown'] = (t.byContract[r.contract || 'unknown'] || 0) + 1;
    if (r.edgeCache && t.edgeCache[r.edgeCache] !== undefined) t.edgeCache[r.edgeCache]++;
    if (r.classifier && t.classifier[r.classifier] !== undefined) t.classifier[r.classifier]++;
    if (typeof r.prefilterWouldSkip === 'boolean') { t.prefilter.evaluated++; if (r.prefilterWouldSkip) t.prefilter.wouldSkip++; }
    if (r.prefilterMiss === true) t.prefilter.miss++;
    ...
    if (r.contract === 'draft' && r.tier === 'primary' && r.effort && typeof r.latency === 'number') {
      ...
      const _full = typeof r.fullDraft === 'boolean' ? r.fullDraft : (typeof r.sysChars === 'number' && r.sysChars >= 15000);
      const _b = _full ? _byEff : _otherEff;
      (_b[r.effort] = _b[r.effort] || []).push(r.latency);
    }
    // Token bands: primary-tier DRAFTS only. Fact probes are small prompts on another cache key and
    // would sit in the pre-M4 band by size alone; a fallback tier is another model and another cache.
    if (r.contract === 'draft' && r.tier === 'primary' && typeof r.promptTokens === 'number' && typeof r.cachedTokens === 'number') {
      const b = (r.sysChars >= PERF_M4_SYSCHARS) ? bands.postM4 : bands.preM4;
      b.cached.push(r.cachedTokens); b.uncached.push(r.promptTokens - r.cachedTokens);
      if (typeof r.writtenTokens === 'number') b.written.push(r.writtenTokens);
    }
  }
  for (const e of Object.keys(_byEff)) t.draftsByEffort[e] = { calls: _byEff[e].length, latencyMedian: med(_byEff[e]) };
  for (const e of Object.keys(_otherEff)) t.otherDraftCallsByEffort[e] = { calls: _otherEff[e].length, latencyMedian: med(_otherEff[e]) };
  if (t.prefilter.evaluated) t.prefilter.wouldSkipRate = Math.round(1000 * t.prefilter.wouldSkip / t.prefilter.evaluated) / 10;
  for (const k of Object.keys(bands)) {
    const b = bands[k];
    t.tokensBySysCharsBand[k] = { label: b.label, calls: b.cached.length,
      cachedMedian: med(b.cached), cachedMean: mean(b.cached), uncachedMedian: med(b.uncached), uncachedMean: mean(b.uncached),
      cacheHits: b.cached.filter(x => x > 0).length, writtenMean: mean(b.written) };
  }
  return t;
}
...
// (v7.73) Does this field hold a JSON object rather than prose? Object only — an array is not an
// email envelope, and a bare string that merely starts with a brace is not either.
function isNestedEnvelope(raw) {
...
// (v7.73) UNWRAP A DOUBLE-ENCODED EMAIL RATHER THAN THROWING AWAY A GOOD ONE.
// MEASURED, NOT ASSUMED. The first afternoon of /degenerate rows (9/8, all gpt-5.6-luna,
// finish=stop, primary, attempt 0) showed FOUR of five rejections carrying 410-468 characters in
// the email field that parse as a complete JSON object with a subject inside — a finished email,
// wrapped one layer too deep. Three of those four ALSO filled the top-level subject correctly, so
// the model wrote the subject twice. Only the fifth (email = the 7-char string "subject") was a
// real abandonment. We were discarding a verified-good email and paying for a second model call.
//
// THE INNER BODY KEY IS NOT GUESSED. The stored samples truncate at 40 characters, so the inner
// key name was never visible and inventing one would be the fourth wrong fix in this area. This
// takes the longest string value that is not the subject, after preferring the conventional names
// when they are present — so it works whatever the model called it.
//
// STRICTLY A RESCUE. It runs only on text that is ABOUT to be rejected, it returns null unless it
// produces a body that passes the ordinary degenerate check on its own, and the caller keeps the
// original text when it returns null. It cannot turn a passing response into a failing one and it
// cannot admit a body the guard would otherwise refuse.
// (v7.79) See the call site. Returns null unless the draft has NO usable top-level subject AND the
// email carries one — as a leading "subject:" line (any case) or as the subject of an email object.
// Only the top-level key is added; nothing else in the draft changes.
function liftEmbeddedSubject(text) {
  let parsed;
  try {
  ...
  if (typeof parsed.email === 'string') {
    const m = parsed.email.match(/^\s*subject[ \t]*:[ \t]*([^\n]+)/i);
    if (m && m[1].trim()) { subject = m[1].trim(); from = 'line'; }
  } else if (parsed.email && typeof parsed.email === 'object' && typeof parsed.email.subject === 'string' && parsed.email.subject.trim()) {
    subject = parsed.email.subject.trim(); from = 'object';
  }
  if (!subject) return null;
  subject = subject.slice(0, 200);
  return { text: JSON.stringify({ ...parsed, subject }), info: { from, chars: subject.length } };
}

function repairNestedEmail(text) {
  let parsed;
  try {
    let t = String(text).trim();
    if (t.startsWith('```')) t = t.replace(/^```(?:json)?\s*/i, '').replace(/```\s*$/, '').trim();
  ...
  if (!isNestedEnvelope(parsed.email)) return null;

  let inner;
  try { inner = JSON.parse(parsed.email.trim()); } catch { return null; }

  const innerSubject = typeof inner.subject === 'string' ? inner.subject.trim() : '';
  let body = '';
  for (const k of ['body', 'email', 'text', 'message', 'content', 'html']) {
    if (typeof inner[k] === 'string' && inner[k].trim()) { body = inner[k].trim(); break; }
  }
  if (!body) {
    for (const [k, v] of Object.entries(inner)) {
      if (k === 'subject' || typeof v !== 'string') continue;
      if (v.trim().length > body.length) body = v.trim();
    }
  }
  if (!body) return null;

  // Rebuild the shape a clean generation produces, so a repaired draft is indistinguishable
  // downstream from one that never needed repairing.
  const rebuilt = innerSubject && !/^subject\s*:/i.test(body)
    ? 'Subject: ' + innerSubject + '\n\n' + body
    : body;
  if (isDegenerateFieldValue(rebuilt, true)) return null;   // no worse than what we rejected

  const out = { ...parsed, email: rebuilt };
  if (innerSubject && !(typeof parsed.subject === 'string' && parsed.subject.trim())) out.subject = innerSubject;

  return {
    text: JSON.stringify(out),
    info: {
      innerKeys:      Object.keys(inner).slice(0, 12).join(','),
      bodyChars:      body.length,
      subjectChars:   innerSubject.length,
      hadTopSubject:  typeof parsed.subject === 'string' && parsed.subject.trim() ? 1 : 0,
    },
  };
}
...
function hasDegenerateField(text) {
  ...
  // (v7.72) THE SHAPE OF THE WHOLE ENVELOPE, NOT JUST THE OFFENDING FIELD. The failure line has
  // named the bad field since v7.47, and after ten days of that the question it cannot answer is
  // the one that matters: when `email` came back as the word "subject", WAS THERE A REAL SUBJECT
  // IN THE SUBJECT FIELD? If yes, the model collapsed two fields into one; if no, it abandoned the
  // envelope entirely. Those are different defects with different fixes, and nothing recorded so
  // far distinguishes them.
  // LENGTHS, NEVER CONTENT — the v9.7.489 posture, same as sysChars/userChars. -1 means the key was
  // absent or not a string, which is itself a distinct finding and must not read as 0 (empty).
  // `keys` is the envelope's own top-level key names: if the model returned {subject, body} instead
  // of our contract, the key names say so and they carry no customer data.
  const _dlen  = (v) => (typeof v === 'string' ? v.length : -1);
  const shape = {
    sms:       _dlen(parsed.sms),
    email:     _dlen(parsed.email),
    subject:   _dlen(parsed.subject),
    voicemail: _dlen(parsed.voicemail),
    emailBody: typeof parsed.email === 'string'
                 ? degenerateFieldBody(parsed.email, true).trim().length : -1,
    keys:      Object.keys(parsed).slice(0, 12).join(','),
    // (v7.73) THE DISCRIMINATOR THE v7.72 TALLY GOT WRONG. That build split the day on "was there
    // a top-level subject", and one afternoon of real rows showed that is the wrong axis: the
    // 9/8 16:35 row was a double-encoded email WITHOUT a top-level subject, and the 13:15 row was
    // a genuine abandonment WITH one. The question that actually separates them is whether the
    // email field is itself a JSON envelope.
    emailIsJson: typeof parsed.email === 'string' && isNestedEnvelope(parsed.email) ? 1 : 0,
  };
  ...
  for (const [name, raw, isEmail] of fields) {
    ...
    if (isDegenerateFieldValue(raw, isEmail))
      return { field: name, sample: degenerateFieldSample(raw, isEmail), shape };
  }
  ...
  if (present > 0 && emptyNames.length === present)
    return { field: 'all-empty', sample: emptyNames.join(','), shape };
...
// ── (v7.75) THE REGEN IS A REPAIR, AND A REPAIR THAT MAKES IT WORSE IS NOT ACCEPTED ──────────
// LIVE, 9/11, request 7e0d2e6b (Roy Ballard, Audi Lafayette, lead via OEM website, agent Jordyn
// Guzman). The trace reads, in order:
//
//   PRIMARY OK  gpt-5.6-luna 4641ms finish=STOP
//   CLASSIFY    sms=NO email=YES phoneOnFile=true
//   REGEN       triggered (email flagged)
...
//   FINAL       regenerated=true flagged=true classifyFailed=false
//
// The EMAIL asked for a phone number we already have, which is exactly what the regen exists to
// strip, and the revision prompt says so in as many words: "remove ONLY the phone-number request
// and keep every other field, sentence, and status fact identical". The model returned a gutted
// draft instead. What reached the agent was an SMS consisting of the customer's first name and the
// signature, and a one-sentence email.
//
// THE CLASSIFY LINE IS THE PROOF THE REGEN CAUSED IT. It reads sms=NO, so the text draft was not
// degenerate when the primary returned. It was degenerate after. The regen destroyed a field it
// was explicitly told not to touch, and the worker took it, because the only test was
// regenResult.ok -- which says the HTTP call succeeded and nothing whatever about the content.
//
// So the safety net could replace a good draft with a worse one and report success. This guard
// makes the replacement conditional on the regen still being plausibly the same message.
//
// WHY THESE TESTS AND NOT A CLASSIFIER. A second model call costs latency on the path that has
// already spent two, and the failure is a SHAPE, not a judgement: the fields are there and they
// are empty. Length against the draft we are replacing answers it without another round trip.
//
// THE TOLERANCES ARE ASYMMETRIC ON PURPOSE. A field the classifier flagged is expected to lose a
// sentence, so it may lose more. A field that was NOT flagged was supposed to come back identical,
// so a large loss there is prima facie evidence the model rewrote what it was told to preserve.
// The absolute floor catches the name-plus-signature shape, which is short whatever the primary
// was. On the incident the text went to 39 characters and the email kept roughly a seventh of its
// length; both tests catch both fields independently.
//
// FAILS TOWARD THE PRIMARY. If the regen cannot be parsed or compared, the primary is kept -- it
// may still carry the phone ask, which is a small defect, where an empty draft is a total one. The
// one case that returns ok without checking is a primary that does not parse, because then there
// is no baseline and rejecting would throw away the only repair available.
const REGEN_MIN_CHARS        = 60;   // below this a field is a greeting and a signature
const REGEN_KEEP_FLAGGED     = 0.40; // a flagged field may lose the phone sentence
const REGEN_KEEP_UNFLAGGED   = 0.60; // an unflagged field was supposed to be untouched
const REGEN_GUARD_FIELDS     = ['sms', 'email', 'voicemail'];
const REGEN_PRESENCE_FIELDS  = ['sms', 'email', 'voicemail', 'subject'];

function regenGuard(primaryText, regenText, flags) {
  const b = safeJsonParse(regenText);
  if (!b) return { ok: false, why: 'regen did not parse as JSON' };
  const a = safeJsonParse(primaryText);
  if (!a) return { ok: true, why: 'primary did not parse — no baseline to compare against' };

  const str = (o, f) => (o && typeof o[f] === 'string') ? o[f].trim() : '';

  // (1) A field that carried something must still carry something. subject is included here and
  // nowhere below: it is short by nature, so a length ratio on it would be noise.
  for (const f of REGEN_PRESENCE_FIELDS) {
    if (str(a, f) && !str(b, f)) return { ok: false, why: f + ' came back empty' };
  }

  // (2) A substantive field must still be substantive, and must not have lost most of itself.
  for (const f of REGEN_GUARD_FIELDS) {
    const before = str(a, f), after = str(b, f);
    if (before.length <= 20) continue;           // the classifier ignores these; so does this
    if (after.length < REGEN_MIN_CHARS) {
      return { ok: false, why: f + ' collapsed to ' + after.length + ' chars (floor ' + REGEN_MIN_CHARS + ')' };
    }
    const floor = (flags && flags[f]) ? REGEN_KEEP_FLAGGED : REGEN_KEEP_UNFLAGGED;
    if (after.length < before.length * floor) {
      const lost = Math.round(100 - (after.length / before.length) * 100);
      return {
        ok: false,
        why: f + ' lost ' + lost + '% of its length (' + before.length + ' -> ' + after.length
           + ', ' + (flags && flags[f] ? 'flagged' : 'unflagged') + ' floor '
...
async function classifyPhoneAsk(messageText, authHeader) {
  ...
  try {
    ...
    // (v7.77) { state: 'yes' | 'no' | 'failed' }, with the HTTP status or the error on a failure.
    // A failure used to return false -- indistinguishable from a real "no". See the CLASSIFY FAILED line.
    if (!res.ok) return { state: 'failed', status: res.status };
    const data = await res.json();
    const answer = (data?.choices?.[0]?.message?.content || '').trim().toUpperCase();
    return { state: answer.startsWith('YES') ? 'yes' : 'no' };
  } catch (e) {
    return { state: 'failed', error: String((e && (e.name === 'AbortError' ? 'timeout' : e.message)) || 'unknown').slice(0, 60) };
  } finally {
    clearTimeout(timer);
  }
}