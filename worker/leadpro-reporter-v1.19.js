 ...
 * v1.18 — THE DISAGREEMENT LIST WAS AN EXCLUSION LIST, so every delta added after it was written
 *   fell straight into it. The filter read `delta !== 'AGREE-COMMITMENT' && delta !== 'AGREE-NONE'`,
 *   which was complete when there were four deltas and silently wrong once there were nine: the
 *   three added since (AGREE-FIRED, PROBE-FAILED, NO-COMPREHENSION-VERDICT) plus NO-REGEX-VERDICT
 *   all landed in a table headed DISAGREEMENTS. Seen live on the 8/22 report, which listed a
 *   genuine AGREE-FIRED row as a disagreement.
 *   FIXED as a POSITIVE list — the three deltas that ARE disagreements, named. A future delta
 *   cannot fall through into it, which is the whole lesson of this bug: an exclusion list is a
 *   promise that nobody will ever add a case, and somebody always does.
 *
 * v1.17 — THE COMPREHENSION SECTION SPLITS BY DETECTOR, for extension v9.7.566 (Phase 3). The
 *   comprehension pass now answers three separate questions — verbal-commit, day-lock and
 *   off-franchise — and merging their series would make each one uninterpretable: they have
 *   different regexes, different failure modes and, right now, the same zero evidence. A row
 *   with no detector is counted as 'verbal-commit', which is what every pre-v9.7.566 row is.
 *   Two new deltas are recognised (AGREE-FIRED, NO-COMPREHENSION-VERDICT), and the section now
 *   separates WOULD HAVE DISAGREED from ACTUALLY CHANGED THE DIRECTIVE via sourceUsed — with all
 *   three authoritative flags off, the second number should be zero, and if it is not, something
 *   is live that should not be.
 *
 * v1.16 — THE COMPREHENSION SERIES IS RETIRED, because none of it was ever a reading. Extension
 *   v9.7.564 traced the 8/21 SAFE_FALLBACK incident to the proxy's MIN_CONTENT_CHARS floor: it is
 *   150, it is written for a three-field customer draft, and the observer probe's CORRECT answer
 *   — {"kind":"none","note":null,"quote":null} — is 40 characters. Every probe answer was rejected
 *   at all three tiers. The SAFE_FALLBACK returned in its place is VALID JSON with no "kind" key,
 *   so the observer read kind:'none' off a canned customer draft and persisted it as agreement.
 *   The floor predates the observer, so this is true of EVERY comprehension row ever collected.
 *   The 100% agreement rate this section has been showing was built entirely from failed probes.
 *
 *   `probeOk === true` is now required for a row to enter any rate. Its absence is not a missing
 *   optional field — it is the signature of the whole fabricated series, and a shape test beats a
 *   version-string comparison. Rows still appear in the delta breakdown so nothing is hidden; they
 *   simply cannot move a number. PROBE-FAILED (v9.7.564 reports it explicitly) gets its own row
 *   and its own tile, and when NOTHING is comparable the section says so in as many words rather
 *   than rendering a confident rate over zero usable rows.
 *
 *   COMPARABLE IS COUNTED, NOT SUBTRACTED. v1.15 derived it as total - noRegex; adding a second
 *   exclusion to that expression would double-count, because a NO-REGEX-VERDICT row can also be
 *   an unverified-probe row and on the pre-fix series most are both. The loop counts it directly.
 *
 * v1.15 — NO-REGEX-VERDICT rows are counted separately and EXCLUDED from both rates. The
 *        verbal-commit regex is gated off on showroom-followup and note-free leads; a row where
 *        it never ran is not agreement and not disagreement, and folding it into either would
 *        silently move the number Phase C is meant to decide on. Pairs with extension v9.7.563
 *        and proxy v7.57.
 *
 * v1.14 — The Commit Comprehension section breaks down BY NOTE TYPE. Extension v9.7.560 widened
 *        both the verbal-commit regex and the observer from [CALL NOTE] to [CALL NOTE] + [NOTE],
 *        and carries the sourcing note type through to KV (proxy v7.56). "These disagree" is far
 *        less actionable than "these disagree on general notes specifically", which is the whole
 *        reason the type is carried. Rows written before v9.7.560 have no type and show as
 *        'unknown' rather than being dropped or silently counted as call notes. The disagreement
 *        list gains a NOTE column for the same reason.
 *
