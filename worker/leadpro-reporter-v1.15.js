...
function buildReport(reqs, dateLabel, feedbackData = null, commitData = null) {
  ...
  const commitHtml = (() => {
    ...
    const noRegex    = cd.deltas['NO-REGEX-VERDICT'] || 0;
    // Rates are over COMPARABLE rows only — rows where both sides actually produced a reading.
    const comparable = cd.total - noRegex;
    ...
    const rows = [
      ...
      ['NO-REGEX-VERDICT',            noRegex,                                  'regex gated off — not a comparison',     '#6b7280'],
    ].map(function (r) {
    ...
    return section('Commit Comprehension (observer) — ' + cd.total + ' verdict' + (cd.total === 1 ? '' : 's'),
      ...
      + (noRegex ? '<div style="height:10px"></div>' + grid(
    ...
    if (env.LEADPRO_LICENSES) {
      ...
      if (ccEntries.length > 0) {
        ...
        const deltas = { 'AGREE-COMMITMENT':0, 'AGREE-NONE':0, 'DISAGREE-REGEX-ONLY':0,
                         'DISAGREE-COMPREHENSION-ONLY':0, 'QUOTE-FABRICATED':0, 'NO-REGEX-VERDICT':0 };
        const bySource = {};
        const disagreements = [];
        ...
        commitData = { date: dateLabel, total: ccEntries.length, deltas, bySource, byNoteType,
