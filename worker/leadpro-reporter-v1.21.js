    ...
    return section('Commit Comprehension (observer) — ' + cd.total + ' verdict' + (cd.total === 1 ? '' : 's'),
      (comparable === 0
        ? '<div style="background:#1f1416;border:1px solid #7f1d1d;border-radius:5px;padding:12px;'
          ...
          + '</div>'
        : grid(tile('Agreement rate', cmpPct(agree), agree + ' of ' + comparable + ' comparable', '#4ade80'),
               tile('Disagreement rate', cmpPct(disagree), disagree + ' of ' + comparable + ' comparable', disagree ? '#fbbf24' : '#4b5563')))
    ...
    if (env.LEADPRO_LICENSES) {
      ...
      if (ccEntries.length > 0) {
        ...
        for (const e of ccEntries) {
          ...
          if (e.delta !== 'NO-REGEX-VERDICT' && e.delta !== 'PROBE-FAILED') comparable++;
