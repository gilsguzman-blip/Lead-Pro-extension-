  ...
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'POST' && url.pathname.endsWith('/commit-comprehension')) {
      ...
      const CC_DELTAS = ['AGREE-COMMITMENT', 'AGREE-NONE', 'DISAGREE-REGEX-ONLY',
                         'DISAGREE-COMPREHENSION-ONLY', 'QUOTE-FABRICATED'];
      ...
      const ccValue = JSON.stringify({
        ...
        compKind:       ['firm', 'soft', 'none'].indexOf(cc.compKind) >= 0 ? cc.compKind : 'none',
      ...
      try {
        ...
        console.log(`[COMMIT-COMP] ${cc.delta} | regex:${cc.regexFired ? 'fired' : 'none'} | comp:${cc.compKind || 'none'} | ${ccMeta.store || '?'}`);
        return corsResponse('{"ok":true}', 200);
      } catch (e) {
    ...
    const reqEffort = (gen.reasoningEffort === 'low' || gen.reasoningEffort === 'medium' || gen.reasoningEffort === 'high')
      ? gen.reasoningEffort
      : REASONING_EFFORT;
    ...
    console.log(`[${requestId}] START tokens=${callerMax} effort=${reqEffort} cacheKey=${cacheKey}`);
...
async function callOpenAI({ spec, messages, maxTokens, authHeader, timeoutMs, cacheKey, reasoningEffort }) {
  ...
  if (cacheKey) {
    ...
    if (!spec.cacheBreakpoints) payload.prompt_cache_retention = CACHE_RETENTION;
  }
  ...
  try {
    ...
    const degenerate = isLikelyJson(text) ? hasDegenerateField(text) : null;
    if (degenerate) {
      return {
        ok: false,
        latency: Date.now() - t0,
