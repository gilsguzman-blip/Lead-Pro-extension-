...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'GET' && url.pathname.endsWith('/degenerate')) {
      ...
      let doubleEncoded = 0, abandoned = 0;
      for (const r of rows) {
        byField[r.field]          = (byField[r.field]          || 0) + 1;
        byModel[r.model]          = (byModel[r.model]          || 0) + 1;
        byFinish[r.finish || '?'] = (byFinish[r.finish || '?'] || 0) + 1;
        byOutcome[r.outcome || 'rejected'] = (byOutcome[r.outcome || 'rejected'] || 0) + 1;
        if (r.field !== 'email') continue;
        // A repaired row is by definition the double-encoded shape — that is the only thing the
        // unwrap rescues — so it counts without needing a shape block.
        if (r.outcome === 'repaired' || (r.shape && r.shape.emailIsJson)) doubleEncoded++;
        else if (r.shape) abandoned++;
      }
      return corsResponse(JSON.stringify({
        date: dateParam, tz: 'America/Chicago', count: rows.length,
        byField, byModel, byFinish,
        byOutcome,
        emailShapes: { doubleEncoded, abandoned },
        rows,
      }), 200);
    }
        ...
        if (result.repaired) {
          try {
            const _rpRow = {
              ...
              latency: result.latency, finish: result.finishReason || '',
