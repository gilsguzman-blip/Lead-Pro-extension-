...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'GET' && url.pathname.endsWith('/degenerate')) {
      ...
      // back alongside it?
      const byField = {}, byModel = {}, byFinish = {};
      let subjectPresent = 0, subjectEmptyOrAbsent = 0;
      for (const r of rows) {
        byField[r.field]         = (byField[r.field]         || 0) + 1;
        byModel[r.model]         = (byModel[r.model]         || 0) + 1;
        byFinish[r.finish || '?'] = (byFinish[r.finish || '?'] || 0) + 1;
        if (r.field === 'email' && r.shape) {
          if (r.shape.subject > 0) subjectPresent++; else subjectEmptyOrAbsent++;
        }
      }
      return corsResponse(JSON.stringify({
        date: dateParam, tz: 'America/Chicago', count: rows.length,
        byField, byModel, byFinish,
        emailRejections: { subjectPresent, subjectEmptyOrAbsent },
        rows,
      }), 200);
    }
