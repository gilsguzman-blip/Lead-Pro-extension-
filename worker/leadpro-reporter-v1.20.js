/**
 * leadpro-reporter v1.20 — A HEALTHY CACHE WAS BEING GRADED AS A FAILING ONE.
 *
 ...
 * ~25% — that tile could not have lit up on ANY day, however well caching worked. A metric that
 * cannot move is worse than no metric: it invites work on a problem that does not exist. Removed
 * rather than re-thresholded. Same shape as the NO-CUSTOMER-TEXT fix in v1.19 the same day: a
 * healthy system reported against a denominator that makes it look broken.
 *
 * THE PROXY HAS BEEN EMITTING THE HONEST NUMBERS SINCE v7.45 and this report was discarding them.
 * Its own header says they exist to "distinguish 'the per-lead prompt just dominates' (expected,
 * unfixable) from 'the prefix is not being cached'". A real line from the 8/23 worker log:
 *
 ...
 *
 * MEASURED across every 8/23 worker log, primary tier, 258 draft requests:
 *   ceiling             24.5%   the most cached/total can ever be
 *   cachedOfSys (warm)    84%   of the CACHEABLE prefix, actually served from cache
 *   warm / cold      67% / 33%
 * One stable cache key across every log; zero "breakpoint NOT applied" warnings. The prefix was
 * being cached correctly the whole time.
 *
 * WHAT THE TILES SAY NOW: 'Of achievable' (hit rate divided by the ceiling — the number to watch),
 * 'Prefix cached (warm)' (of the cacheable part, how much came back — a low figure here means the
 * prefix boundary moved, not that the prompt is big), 'Ceiling', and 'Cold starts' with a
 * percentage. On 8/23's numbers the headline moves from a damning 9.8% to an honest 40% of
 * achievable. Every new value has an N/A path, and the ceiling divisor is guarded, so a day of
 * pre-v7.45 log lines degrades rather than crashing.
 *
 * A DEFECT FOUND WHILE SHIPPING THIS, and it is the same lesson one layer out: removing
 * hotCount's definition left one live reference in the plain-text summary, so buildReport threw
 * ReferenceError and the entire report died — while the new cache-ceiling suite sat at 17/17
 * green, because every assertion in it scans SOURCE TEXT. commit-persistence caught it, because
 * it RUNS buildReport. Same class as the v7.64 proxy crash: a regex proves a string is in a file,
 * not that the file runs. The orphan is now pinned directly, and the text summary carries the
 * same honest figures as the tiles.
 *
 * NOT CHANGED, deliberately: the 30m TTL. 33% of primary requests start cold — about 283k
 * avoidable input tokens a day — but '30m' is the documented supported value for the 5.6 family
 * and v7.61 records that an unsupported ttl risks a 400 on the PRIMARY tier. Trading a working
 * ladder for that much is the wrong trade. Reporter-only build: no proxy or extension change.
 *
 * Builds on v1.19.
 */
    ...
    return section('Commit Comprehension (observer) — ' + cd.total + ' verdict' + (cd.total === 1 ? '' : 's'),
      ...
      + (cd.byDetector && Object.keys(cd.byDetector).length
          ? '<div style="font-size:11px;color:#6b7280;margin-bottom:6px">BY DETECTOR — '
            ...
            + table(['Detector', 'Rows', 'Probe OK', 'Agree', 'Disagree', 'Fabricated', 'Changed'],
                Object.keys(cd.byDetector).sort().map(function (k) {
                  const b = cd.byDetector[k];
                  return tr([k, String(b.total), String(b.usable), String(b.agree), String(b.disagree),
                             String(b.fabricated),
    ...
    if (env.LEADPRO_LICENSES) {
      ...
      if (ccEntries.length > 0) {
        ...
        for (const e of ccEntries) {
          const _det = ['verbal-commit','day-lock','off-franchise'].indexOf(e.detector) >= 0
            ? e.detector : 'verbal-commit';
        ...
        commitData = { date: dateLabel, total: ccEntries.length, deltas, verifiedDeltas, bySource,
                       byNoteType, byDetector, disagreements, regexFired, compFired,
