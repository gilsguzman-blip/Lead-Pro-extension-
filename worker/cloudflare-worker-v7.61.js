...
const TIER_EFFORTS = {
  ...
  'gpt-5.4-nano-2026-03-17': ['low', 'medium', 'high'],
...
async function callOpenAI({ spec, messages, maxTokens, authHeader, timeoutMs, cacheKey, reasoningEffort, contract }) {
  ...
  if (spec.gpt5) {
    ...
    const _eff = resolveEffort(spec.model, reasoningEffort);
  ...
  if (cacheKey) {
    ...
    if (spec.cacheBreakpoints) {
      ...
      payload.prompt_cache_options = { ttl: CACHE_TTL };
    }
  }
  ...
  try {
    const res = await fetch(OPENAI_ENDPOINT, {
      ...
      body: JSON.stringify({
        ...
        // (v7.61) Same field swap as the main path. Still inert here for the reason above.
