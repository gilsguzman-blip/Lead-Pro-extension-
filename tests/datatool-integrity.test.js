...
// ── (2026-09-21) THE DEALER FILTER, RUN FOR REAL ────────────────────────────────────────────
// Gil, on a 2026-09-21 Audi Lafayette pair: 252 rows read, 0 units kept, 252 other rooftops, 0
// stores — and nothing on screen said why. The deployed source was byte-for-byte the repo's, so
// the tool had not changed; the dealer name in the export had. The tile could report only a count.
//
// The asymmetry that allowed it: get() has always matched the COLUMN name case- and
// whitespace-insensitively, while the dealer VALUE was compared with `in` against raw map keys.
// These assertions execute the resolver rather than scanning for it, and they pin BOTH directions
// — what must resolve, and what must not, because a filter that accepts everything is not a filter.
console.log('\nthe dealer filter resolves names the way get() resolves columns:');

const rStart = src.indexOf('const DEALER_MAP=');
const rEnd = src.indexOf('function resolveDealer(raw)');
check('the resolver is present in the source', rStart > -1 && rEnd > rStart, true);
const rEndFull = src.indexOf('}', src.indexOf('return Object.prototype.hasOwnProperty.call(DEALER_NORM,n)', rEnd)) + 1;
const rsb = {};
vm.createContext(rsb);
vm.runInContext(src.slice(rStart, rEndFull), rsb, { filename: 'resolver' });
const resolve = (v) => { const h = vm.runInContext('resolveDealer', rsb)(v); return h ? h[0] : null; };

check('the five vAuto dealer names still resolve, unchanged',
  ['Community Toyota TX', 'Community Kia TX', 'Community Honda TX',
   'Community Honda of Lafayette', 'Audi Lafayette'].map(resolve),
  ['6189', '6190', '6191', '24399', '21135']);
check('case and spacing no longer decide it',
  ['AUDI   LAFAYETTE', '  Audi Lafayette  ', 'community honda tx'].map(resolve),
  ['21135', '21135', '6191']);
// The shape from the 9/21 filename, "CC-Audi Lafayette". A SHAPE, not a list of prefixes — an
// enumeration of known prefixes is how the next one walks through.
check('a short export prefix is stripped',
  ['CC-Audi Lafayette', 'cc - audi lafayette', 'CC-Community Honda TX'].map(resolve),
  ['21135', '21135', '6191']);
check('the stores\' own display labels resolve too',
  ['Honda Lafayette', 'Toyota Baytown', 'Kia Baytown'].map(resolve),
  ['24399', '6189', '6190']);

console.log('\n  ...and it still refuses everything else:');
check('a genuinely different rooftop does not resolve',
  ['Gulf Coast Nissan', 'Community Ford TX', 'Lafayette Motors'].map(resolve), [null, null, null]);
check('empty, null and undefined do not resolve',
  ['', null, undefined].map(resolve), [null, null, null]);
// The prefix rule must not eat a real name: a long leading token is not a prefix.
check('a long leading token is left alone, so it cannot manufacture a match',
  resolve('Community-Audi Lafayette'), null);

console.log('\n  the screen can now say what it rejected:');
check('the normalize path records unmatched dealer names',
  /unmatched\[dealer\]=\(unmatched\[dealer\]\|\|0\)\+1/.test(src), true);
check('a missing Dealer Name column is counted separately from an unknown value',
  /stats\.noColumn\+\+/.test(src) && /dealer==null/.test(src), true);
check('there is an element to render them into',
  /id="inv-unmatched"/.test(src), true);
// Dealer names come from a file the browser just read; they are rendered as TEXT, never as HTML.
check('the rejected values are written with textContent, not innerHTML',
  /um\.textContent=parts\.join/.test(src) && !/inv-unmatched'\)\.innerHTML/.test(src), true);
check('the count still reports every rejected row, however it was rejected',
  /if\(dealer==null\)\{stats\.noColumn\+\+;stats\.other\+\+;continue;\}/.test(src), true);
