...
for t in amp-banner arc-bound bereavement delivery-match arc-relevancy commit-comprehension crm-entry-walk \
         ...
         sched-attribution spouse-attribution fact-comprehension cadence arc-state stalled-phase edge-bypass value-fact-diag on-premise-authorship sched-scan-depth transcript-cutoff tapback-anchor anchor-authorship reply-vs-inquiry close-out-floor own-words-topics close-out-eligibility customer-facing-hygiene consent-and-stock declined-alternative phone-directory vm-lead-scope inventory-freshness authoritative-phase scrub-diacritics distance-appt-gate pause-supersession color-ask \
