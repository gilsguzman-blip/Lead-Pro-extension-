...
(function() {
  ...
  const vehicleRaw = firstId([
    ...
    '.leadinfodetails',
    '[data-label="Vehicle"]'
  ]) || labelValue('Vehicle') || labelValue('Vehicle Info');
  ...
  // Store's home state — used to detect out-of-state buyers
  var storeStateLower = (store || '').toLowerCase();
  var LA_DEALER_IDS = ['24399', '21135'];
  var storeState = LA_DEALER_IDS.indexOf(String(dealerId || dealerIdFromUrl || '')) !== -1 ? 'LA' : 'TX';
  var isDistanceBuyer = !!(customerState && customerState !== storeState);
  ...
  if (usefulFrame && typeof MutationObserver !== 'undefined') {
    ...
    var _lpMutationObserver = new MutationObserver(function(mutations) {
      ...
      _lpMutationTimer = setTimeout(function() {
        ...
        if (noteCount !== _lpLastNoteCount) {
          ...
          try { chrome.runtime.sendMessage({ type: 'LEADPRO_DOM_UPDATED', noteCount: noteCount }); } catch(e) { console.warn('[Lead Pro] scraper error:', e.message); }
        }
      }, 500);
    });
    ...
    window.addEventListener('beforeunload', function() {
      if (_lpMutationObserver) { _lpMutationObserver.disconnect(); _lpMutationObserver = null; }
      if (_lpMutationTimer) { clearTimeout(_lpMutationTimer); _lpMutationTimer = null; }
    });
  }
